...
static bool processSectors(wadfile_t *wadfile, uint32_t lumpNum);
...
// used to check data sizes
int maxLineFlag = 0;
int maxLineSpecial = 0;
int maxLineTag = 0;
int maxLineFrontSide = 0;
int maxLines = 0;
//
int maxPatchWidth = 0;
int maxPatchHeight = 0;
int maxPatchOffsetX = 0;
int maxPatchOffsetY = 0;
int minPatchOffsetX = 0;
int minPatchOffsetY = 0;
int lastSpriteLump = 0;
int firstNonScreenPatch = 0;
//
int maxSectorTag = 0;
int maxSectorReducedTag = 0;
int maxSectorSpecial = 0;
int currentTag = 0;
int tagmap[65536] = {0};
// used to profile data size
void getMax(int newValue, int *oldValue)
{
    if (newValue > *oldValue)
        *oldValue = newValue;
}
void getMin(int newValue, int *oldValue)
{
    if (newValue < *oldValue)
        *oldValue = newValue;
}
void printProfiledData()
{
    #if PROFILE_DATA_SIZE
        printf("Max Line Lines: %d\r\n", maxLines);
        printf("Max Line Flag: %d\r\n", maxLineFlag);
        printf("Max Line Special: %d\r\n", maxLineSpecial);
        printf("Max Line Tag: %d\r\n", maxLineTag);
        printf("Max Line FrontSide: %d\r\n", maxLineFrontSide);
        // sprite and patch data
        printf("Last Sprite Lump: %d\r\n", lastSpriteLump);
        printf("First patch of interest %d\r\n", firstNonScreenPatch);
        //
        printf("Max patch width: %d\r\n", maxPatchWidth);
        printf("Max patch height: %d\r\n", maxPatchHeight);
        printf("Max offset X: %d\r\n", maxPatchOffsetX);
        printf("Max offset Y: %d\r\n", maxPatchOffsetY);
        printf("Min offset X: %d\r\n", minPatchOffsetX);
        printf("Min offset Y: %d\r\n", minPatchOffsetY);
        //
        printf("Max Sector Tag: %d\r\n", maxSectorTag);
        printf("Max Sector Tag - reduced: %d\r\n", maxSectorReducedTag);
        printf("Max Sector Special: %d\r\n", maxSectorSpecial);
    #endif // PROFILE_DATA_SIZE
}
bool processWad(wadfile_t *wadfile, bool removeSound, bool convertPatches)
{
    bool res;
    // reset tag map
    memset(tagmap, -1, sizeof(tagmap));
    ...
    else
    {
       // for each patch, change it. Note, marksers are skipped.
        ...
        // for profiling
        lastSpriteLump =  getLumpNumByName(wadfile, "S_END");
        firstNonScreenPatch = getLumpNumByName(wadfile, "AMMNUM0");
...
int getTag(int oldTag)
{
    // tag already allocated ?
    if (oldTag == 667 || oldTag == 666)
    {
        printf(">>>>>FOUND TAG %d<<<<<<<<<<<<<<\r\n", oldTag);
    ...
    if (tagmap[oldTag] != -1)
    {
        return tagmap[oldTag];
    }
    else
    {
        // allocate tag
        tagmap[oldTag] = currentTag;
        currentTag++;
        return tagmap[oldTag];
    }
}
...
bool processLevel(wadfile_t *wadfile, uint32_t lumpNum)
{
    ...
    processSectors(wadfile, lumpNum);
    return true;
}
...
bool processLines(wadfile_t *wadfile, uint32_t lumpNum)
{
    ...
    for(unsigned int i = 0; i < lineCount; i++)
    {
        ...
        getMax(oldLines[i].special, &maxLineSpecial);
        newLines[i].flags = oldLines[i].flags;
        getMax(oldLines[i].flags, &maxLineFlag);
        newLines[i].tag = oldLines[i].tag;
        getMax(oldLines[i].tag, &maxLineTag);
        ...
        getMax(oldLines[i].sidenum[0], &maxLineFrontSide);
...
bool processSectors(wadfile_t *wadfile, uint32_t lumpNum)
{
    uint32_t sectorsLumpNum = lumpNum + ML_SECTORS;

    lump_t *sectors;

    sectors = getLumpByNum(wadfile, sectorsLumpNum);

    if (NULL == sectors)
        return false;

    if(sectors->lump.size == 0)
        return false;

    uint32_t sectorCount = sectors->lump.size / sizeof(mapsector_t);

    mapsector_t *ms = (mapsector_t*) sectors->data;
    for (int i = 0; i < sectorCount; i++)
    {
        getMax(ms[i].tag, &maxSectorTag);
        getMax(getTag(ms[i].tag), &maxSectorReducedTag);
        getMax(ms[i].special, &maxSectorSpecial);
    }
    return true;
}
...
static bool processPatches(wadfile_t *wadfile, int firstLump, int lastLump)
{
    ...
    for(uint32_t patchLumpNum = firstLump; patchLumpNum < lastLump; patchLumpNum++)
    {
        ...
        //
        // for profiling
        if (patchLumpNum < lastSpriteLump && patchLumpNum >= firstNonScreenPatch )
        {
            getMax(newPatch->height, &maxPatchHeight);
            getMax(newPatch->width, &maxPatchWidth);
            getMax(newPatch->leftoffset, &maxPatchOffsetX);
            getMax(newPatch->topoffset, &maxPatchOffsetY);
            getMin(newPatch->leftoffset, &minPatchOffsetX);
            getMin(newPatch->topoffset, &minPatchOffsetY);
        }
        //
