 ...
 * IMPORTANT - READ ME!
 *
 * The RADIO Subsystem requires that the correct radio parameters are set.
 * These parameters are module-dependent, so you can't use the same settings
 * for the Sparkfun MGM240P Matter board (which uses MGM240PB32VNA) and the
 * Arduino Nano Matter Board (which uses the MGM240SD22VNA).
 *
 * If you change the "BOARD symbol below", you MUST double click on the slcp
 * file in Simplicity Studio, and on "Target and Tool Settings" pane, make sure
 * that the correct part number is selected. If not, scroll down, and click on
 * "Change Target/SDK/Generators and under "PART" insert the correct part number:
 * - Arduino Nano Matter:  MGM240SD22VNA
 * - Sparkfun MGM240 Matter: MGM240PB32VNA
 *
 * After that click on "SAVE".
 * A window will appear, and make sure that all files are marked with:
 * "keep my files", then confirm.
 * Then in the "Project Details" pane, click on the three dots "..." and then,
 * on the popup menu, click "Force Generation".
 *
...
// Select board here if not defined by build config
#ifndef BOARD
#define BOARD                              BOARD_SPARKDOOM // choose from: BOARD_SPARKDOOM or BOARD_WSTK_3, or BOARD_SPARKDOOM_ARDUINONANOMATTER, or BOARD_THEGAMEPAD_ARDUINONANOMATTER
#endif
//
#if (BOARD == BOARD_SPARKDOOM || BOARD == BOARD_WSTK_3) && MGM240SD22VNA
    #error You selected to use an Arduino Nano Matter Board in "boards.h" or in the build config but you did not select in the slcp project the MGM240SD22VNA part number. Please refer to the "IMPORTANT READ ME" comment on boards.h to fix this issue!
#endif
#if (BOARD != BOARD_SPARKDOOM_ARDUINONANOMATTER && BOARD != BOARD_THEGAMEPAD_ARDUINONANOMATTER) && MGM240SD22VNA
    #error You selected to use a Sparkfun MGM240P Matter Board in "boards.h" or in the build config but you did not select in the slcp project the MGM240PB32VNA part number. Please refer to the "IMPORTANT READ ME" comment on boards.h to fix this issue!
#endif
//
...
#define GPIO_PORT_D                     3
  ...
  #define VCOM_RX_PIN                           4
  // audio config
  #define STEREO_AUDIO                          1
  #define AUDIO_MODE                            DAC_AUDIO_MODE
  //
  #define AUDIO_PORT_L                          (gpioPortA)
  #define AUDIO_PIN_L                           5
  //
  #define AUDIO_PORT_R                          (gpioPortA)
  #define AUDIO_PIN_R                           4
