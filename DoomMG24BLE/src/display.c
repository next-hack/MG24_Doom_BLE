...
#endif
    GPIO_PinModeSet(DISPLAY_MOSI_PORT, DISPLAY_MOSI_PIN, gpioModePushPull, 1);
    GPIO_PinModeSet(DISPLAY_SCK_PORT, DISPLAY_SCK_PIN, gpioModePushPull, 1);
    //
    for (int i = gpioPortA; i <= gpioPortD; i++)
    {
      GPIO_SlewrateSet(i, 7, 7);
    }
    //
    DISPLAY_NCS_HIGH();
    DISPLAY_DC_HIGH();
