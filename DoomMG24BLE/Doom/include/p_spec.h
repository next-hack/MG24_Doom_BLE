...
typedef struct
{
    ...
    short btexture;
    byte where;
    byte btimer;        // 2022-02-24 next-hack: btimer is 35 anyway...
} button_t;
...
typedef struct
{
// next-hack: why shall we use so much space_
    //int picnum :12;
    uint16_t basepic :12;
    //int numpics :10;
    uint16_t numpics :3;        // next-hack: for each anim, we have at most 4 frames...
    //short speed :15;          // next-hack: this is constant to 8. In P_UpdateSpecials() it was already hardcoded in a bad way anyway.
    uint16_t istexture :1;
...
typedef struct
{
    ...
    char startname[9];
    // short speed;    // next-hack: this is constant to 8. In P_UpdateSpecials() it was already hardcoded in a bad way anyway.
