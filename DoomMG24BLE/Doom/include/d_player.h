...
typedef struct
{
 //next-hack: using bitfields for realistic value. One is not going to play more than 24 hours and you can't really have so many secrets
    // Player stats, kills, collected items etc.
    uint32_t ssecret : 8;    // 256 secrets ? enough
    int stime: 24;              // 8 million ticks = 2.6 days
    uint32_t in: 1;  // whether the player is in game
    int skills : 15;
    int sitems;
    short frags[4];
} wbplayerstruct_t;
