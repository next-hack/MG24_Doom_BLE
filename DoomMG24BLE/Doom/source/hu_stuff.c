...
// font sets. next-hack: this steals some bytes. let's put in flash.
...
void HU_Init(void)
{
    ...
    int j;
    char buffer[9];
    hu_font = Z_Calloc(sizeof (*hu_font), HU_FONTSIZE, PU_STATIC,  NULL);
    ...
    void *bufferToFree = hu_font;
    hu_font = writeBufferToFlashRegion(hu_font, sizeof (*hu_font) * HU_FONTSIZE, FLASH_IMMUTABLE_REGION, true);
    Z_Free(bufferToFree);
}
...
void HU_Drawer(void)
{
    ...
    else
    {
      ...
      {
        hu_textline_t line;
        HUlib_initTextLine(&line, HU_TITLEX, HU_TITLEY, (const patch_t*) hu_font, HU_FONTSTART);
        snprintf(line.l , sizeof(line.l), "fps: %02d.%01d", _g->fps_framerate / 10, _g->fps_framerate % 10);
        line.needsupdate = 4;
        line.linelen = strlen(line.l);
        line.len = line.linelen;
