    ...
    if (priority < 8)
    {
        if (_g->plyr->damagecount && _g->plyr->attacker && _g->plyr->attacker != _g->plyr->mo)
        {
            ...
            else
            {
                ...
                if (badguyangle > ANGLE16_TO_ANGLE32(_g->plyr->mo->angle16))
                {
                    // whether right or left
                    diffang = badguyangle - ANGLE16_TO_ANGLE32(_g->plyr->mo->angle16);
                    i = diffang > ANG180;
                }
                else
                {
                    // whether left or right
                    diffang = ANGLE16_TO_ANGLE32(_g->plyr->mo->angle16) - badguyangle;
                    i = diffang <= ANG180;
                } // confusing, aint it?
    ...
    if (statusbaron)
    {
        ...
        else if (_g->st_needrefresh == 1)
        {
           ...
           for (int i = 0; i < ST_SCALED_HEIGHT * SCREENPITCH / 4; i++)
             *d++ = *s++;
#else
           fast32BytesCopy(d, s, SCREENWIDTH);
#endif
           _g->st_needrefresh = 0;
        }
    }
...
static void ST_loadGraphics(boolean doload)
{
    ...
    for (i = 0; i < 10; i++)
    {
      ...
      //sprintf(namebuf, "STTNUM%d", i);
        snprintf(namebuf, sizeof(namebuf), "STTNUM%d", i); // regular size for 320-wide number
#else
...
static void ST_initData(void)
{
    ...
    _g->plyr = &_g->players[_g->displayplayer];            // killough 3/7/98
...
static void ST_createWidgets(void)
{
    ...
    ST_HEALTHX,
    ST_HEALTHY, p_wad_immutable_flash_data->tallnum, &_g->plyr->health, &_g->st_statusbaron);

    // armor percentage - should be colored later
    STlib_initPercent(&_g->st_armor,
    ST_ARMORX,
    ST_ARMORY, p_wad_immutable_flash_data->tallnum, &_g->plyr->armorpoints, &_g->st_statusbaron);
    ...
    for (i = 0; i < 6; i++)
    {
        ...
        ST_ARMSY + (i / 3) * ST_ARMSYSPACE, MULTIICON_ARMS + i, &_g->plyr->weaponowned[i + 1], &_g->st_statusbaron);
    }

    // keyboxes 0-2
    STlib_initMultIcon(&_g->w_keyboxes[0],
    ST_KEY0X,
    ST_KEY0Y, MULTIICON_KEYS, &_g->keyboxes[0], &_g->st_statusbaron);

    STlib_initMultIcon(&_g->w_keyboxes[1],
    ST_KEY1X,
    ST_KEY1Y, MULTIICON_KEYS, &_g->keyboxes[1], &_g->st_statusbaron);

    STlib_initMultIcon(&_g->w_keyboxes[2],
    ST_KEY2X,
    ST_KEY2Y, MULTIICON_KEYS, &_g->keyboxes[2], &_g->st_statusbaron);
    ...
    ST_FACESX,
    ST_FACESY, MULTIICON_FACES, &_g->st_faceindex, &_g->st_statusbaron);
}
