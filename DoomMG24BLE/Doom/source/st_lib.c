 ...
 * in external flash. Optimized also RAM usage.
 *
 *-----------------------------------------------------------------------------*/
...
void STlib_initNum(st_number_t *n, int x, int y, const patch_t **pl, short *num, boolean *on, int width)
{
    ...
    n->on = on;
    //n->p = pl;
    n->isTallNum = pl == p_wad_immutable_flash_data->tallnum;
}
...
static void STlib_drawNum(st_number_t *n, int cm, boolean refresh)
{
    ...
    int num = *n->num;
    const patch_t ** p = n->isTallNum ? p_wad_immutable_flash_data->tallnum : p_wad_immutable_flash_data->shortnum;
    int w;
    if (isOnExternalFlash(&p[0]->width))
    {
        w = extMemFlashGetShortFromAddress(&p[0]->width);
    }
    else
        w = p[0]->width;
    int x = n->x;
    ...
    if (!num)
        ...
        V_DrawPatchNoScale(x - w, n->y, p[0]);
    ...
    while (num && numdigits--)
    {
        // CPhipps - patch drawing updated, reformatted
        x -= w;
        V_DrawPatchNoScale(x, n->y, p[num % 10]);
        num /= 10;
    }
}
...
*/
    ...
    switch (mi->pn)
    {
        case MULTIICON_NONE:
            return;
        case MULTIICON_FACES:
            p = p_wad_immutable_flash_data->faces;
            break;
        case MULTIICON_KEYS:
            p = p_wad_immutable_flash_data->keys;
            break;
        default:
            p = p_wad_immutable_flash_data->arms[mi->pn - MULTIICON_ARMS];
            break;
    }
    if (*mi->inum != -1)  // killough 2/16/98: redraw only if != -1
        V_DrawPatchNoScale(mi->x, mi->y, p[*mi->inum]);
...
void ST_refreshBackground(void)
{
    if (_g->st_statusbaron)
    {
        ...
        /*for (uint32_t i = 0; i < 32 * SCREENWIDTH / sizeof(uint32_t); i++)
        {
            *d++ = *s++;
        }*/
