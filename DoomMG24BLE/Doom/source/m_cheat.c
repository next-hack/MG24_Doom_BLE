...
#define plyr (_g->players + _g->consoleplayer)     /* the console player */
...
{
    for (uint32_t i = 0; i < num_cheats; i++)
    {
...
{
    P_GivePower(plyr, pw_invisibility);
}

static void cheat_map()
{
    P_GivePower(plyr, pw_allmap);
}

static void cheat_goggles()
{
    P_GivePower(plyr, pw_infrared);
}
...
{
  plyr->cheats ^= CF_ENEMY_ROCKETS;

  if (plyr->cheats & CF_ENEMY_ROCKETS)
  {
    plyr->health = god_health;
    ...
    plyr->pendingweapon = wp_missile;

    plyr->message = STSTR_ROCKETON;
  }
  else
  {
    plyr->message = STSTR_ROCKETOFF;
  }
}
...
static void cheat_fps()
{
    ...
    if (_g->fps_show)
    {
      plyr->message = STSTR_FPSON;
    }
    else
    {
      plyr->message = STSTR_FPSOFF;
    }
}
