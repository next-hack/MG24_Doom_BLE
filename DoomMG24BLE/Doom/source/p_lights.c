 ...
 *
 *  next-hack: modified code to work with short pointers and other optimizations.
 *
 *-----------------------------------------------------------------------------*/
...
// Returns nothing
//
void THINKFUN_ALIGN4 T_FireFlicker(fireflicker_t *flick)
{
    int amount;
    ...
    if (getRamSector(flick->sector)->lightlevel - amount < flick->minlight)
        getRamSector(flick->sector)->lightlevel = flick->minlight;
    else
        getRamSector(flick->sector)->lightlevel = flick->maxlight - amount;
...
// Returns nothing
//
void THINKFUN_ALIGN4 T_LightFlash(lightflash_t *flash)
{
    if (--flash->count)
        return;

    if (getRamSector(flash->sector)->lightlevel == flash->maxlight)
    {
        getRamSector(flash->sector)->lightlevel = flash->minlight;
        flash->count = (P_Random() & flash->mintime) + 1;
    }
    else
    {
        getRamSector(flash->sector)->lightlevel = flash->maxlight;
...
// Returns nothing
//
void THINKFUN_ALIGN4 T_StrobeFlash(strobe_t *flash)
{
    if (--flash->count)
        return;

    if (getRamSector(flash->sector)->lightlevel == flash->minlight)
    {
        getRamSector(flash->sector)->lightlevel = flash->maxlight;
...
void THINKFUN_ALIGN4 T_Glow(glow_t *g)
{
    switch (g->direction)
    {
        case -1:
            // light dims
            getRamSector(g->sector)->lightlevel -= GLOWSPEED;
            if (getRamSector(g->sector)->lightlevel <= g->minlight)
            {
                getRamSector(g->sector)->lightlevel += GLOWSPEED;
                g->direction = 1;
            }
        ...
        case 1:
            // light brightens
            getRamSector(g->sector)->lightlevel += GLOWSPEED;
            if (getRamSector(g->sector)->lightlevel >= g->maxlight)
            {
                getRamSector(g->sector)->lightlevel -= GLOWSPEED;
                g->direction = -1;
            }
...
void P_SpawnFireFlicker(sector_t *sector)
{
    ...
    getRamSector(sector)->special &= ~31; //jff 3/14/98 clear non-generalized sector type
    ...
    flick->thinker.function_idx = THINKER_IDX(T_FireFlicker);
    flick->sector = sector;
    flick->maxlight = getRamSector(sector)->lightlevel;
    flick->minlight = P_FindMinSurroundingLight(sector, getRamSector(sector)->lightlevel) + 16;
    flick->count = 4;
}
...
void P_SpawnLightFlash(sector_t *sector)
{
    ...
    flash->thinker.function_idx = THINKER_IDX(T_LightFlash);
    flash->sector = sector;
    flash->maxlight = getRamSector(sector)->lightlevel;

    flash->minlight = P_FindMinSurroundingLight(sector, getRamSector(sector)->lightlevel);
    flash->maxtime = 64;
    flash->mintime = 7;
    flash->count = (P_Random() & flash->maxtime) + 1;
}
...
void P_SpawnStrobeFlash(sector_t *sector, int fastOrSlow, int inSync)
{
    ...
    flash->thinker.function_idx = THINKER_IDX(T_StrobeFlash);
    flash->maxlight = getRamSector(sector)->lightlevel;
    flash->minlight = P_FindMinSurroundingLight(sector, getRamSector(sector)->lightlevel);
...
void P_SpawnGlowingLight(sector_t *sector)
{
    ...
    g->sector = sector;
    g->minlight = P_FindMinSurroundingLight(sector, getRamSector(sector)->lightlevel);
    g->maxlight = getRamSector(sector)->lightlevel;
    g->thinker.function_idx = THINKER_IDX(T_Glow);
    g->direction = -1;
...
int EV_TurnTagLightsOff(const line_t *line)
{
    ...
    for (j = -1; (j = P_FindSectorFromLineTag(line, j)) >= 0;)
    {
        ...
        int i, min = getRamSector(sector)->lightlevel;
        // find min neighbor light level
        for (i = 0; i < sector->linecount; i++)
            if ((tsec = getNextSector(getSectorLineByIndex(sector, i), sector)) && getRamSector(tsec)->lightlevel < min)
                min = getRamSector(tsec)->lightlevel;
        getRamSector(sector)->lightlevel = min;
    }
    return 1;
}
...
int EV_LightTurnOn(const line_t *line, int bright)
{
    ...
    for (i = -1; (i = P_FindSectorFromLineTag(line, i)) >= 0;)
    {
        ...
        if (!bright)
            ...
            {
                if ((temp = getNextSector(getSectorLineByIndex(sector, j), sector)) && getRamSector(temp)->lightlevel > tbright)
                {
                    tbright = getRamSector(temp)->lightlevel;
                }
                getRamSector(sector)->lightlevel = tbright;
            }
    }
    return 1;
}
...
int EV_LightTurnOnPartway(const line_t *line, fixed_t level)
{
    ...
    {
        sector_t *temp, *sector = _g->sectors + i;
        int j, bright = 0, min = getRamSector(sector)->lightlevel;

        for (j = 0; j < sector->linecount; j++)
            if ((temp = getNextSector(getSectorLineByIndex(sector, j), sector)))
            {
                if (getRamSector(temp)->lightlevel > bright)
                    bright = getRamSector(temp)->lightlevel;
                if (getRamSector(temp)->lightlevel < min)
                    min = getRamSector(temp)->lightlevel;
            }

        getRamSector(sector)->lightlevel =   // Set level in-between extremes
