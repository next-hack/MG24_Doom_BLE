...
static void P_BringUpWeapon(player_t *player)
{
    ...
    WEAPONBOTTOM + (demo_compatibility ? 0 : FRACUNIT * 2); // next-hack modfied for demo compatibility.
...
const byte weapon_preferences[NUMWEAPONS + 1] =
...
{
    const byte *prefer = &weapon_preferences[0]; // killough 3/22/98
...
void A_Punch(player_t *player, pspdef_t *psp)
{
    ...
    angle = ANGLE16_TO_ANGLE32(player->mo->angle16);
    ...
    player->mo->angle16 = ANGLE32_TO_ANGLE16(R_PointToAngle2(player->mo->x, player->mo->y, _g->linetarget->x, _g->linetarget->y));
}
...
void A_Saw(player_t *player, pspdef_t *psp)
{
    ...
    angle_t angle = ANGLE16_TO_ANGLE32(player->mo->angle16);
    // killough 5/5/98: remove dependence on order of evaluation:
    int t = P_Random();
    angle += (t - P_Random()) << 18;
    ...
    if (angle - ANGLE16_TO_ANGLE32(player->mo->angle16) > ANG180)
    {
        if ( (signed int) (angle - ANGLE16_TO_ANGLE32(player->mo->angle16)) < -ANG90 / 20)
            player->mo->angle16 = ANGLE32_TO_ANGLE16(angle + ANG90 / 21);
        else
            player->mo->angle16 -= ANGLE32_TO_ANGLE16(ANG90 / 20);
    }
    else
    {
        if (angle - ANGLE16_TO_ANGLE32(player->mo->angle16) > ANG90 / 20)
            player->mo->angle16 = ANGLE32_TO_ANGLE16(angle - ANG90 / 21);
        else
            player->mo->angle16 += ANGLE32_TO_ANGLE16(ANG90 / 20);
    }

    setMobjFlagsBits(player->mo,  MF_JUSTATTACKED, SET_FLAGS);
}
...
{
    angle_t an = ANGLE16_TO_ANGLE32(mo->angle16);    // see which target is to be aimed at
...
static void P_GunShot(mobj_t *mo, boolean accurate)
{
    ...
    angle_t angle = ANGLE16_TO_ANGLE32(mo->angle16);
...
void A_BFGSpray(mobj_t *mo)
{
    ...
    {
        int j, damage;
        angle_t an = ANGLE16_TO_ANGLE32(mo->angle16) - ANG90 / 2 + ANG90 / 40 * i;
        ...
        P_SpawnMobj(_g->linetarget->x, _g->linetarget->y, FIXED_Z_TO_FIXED32(_g->linetarget->zr) + (getMobjHeight(_g->linetarget) >> 2), MT_EXTRABFG);
