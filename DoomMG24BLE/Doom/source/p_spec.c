 ...
 *                added caching to flash support. Other optimizations as well.
 *
 *-----------------------------------------------------------------------------*/
...
//  the WAD file.
//
// next-hack: removed speed. This is constant to 8. In P_UpdateSpecials() it was already hardcoded in a bad way anyway.
    ...
    for (i = 0; animdefs[i].istexture != -1; i++)
    {
        ...
        if (animdefs[i].istexture)
        {
            ...
            if (R_CheckTextureNumForName(animdefs[i].startname) == -1)
                continue;
            picnum = R_CheckTextureNumForName(animdefs[i].endname);
            lastanim->basepic = R_CheckTextureNumForName(animdefs[i].startname);
        }
        else
        {
            ...
            for (int flatpicnum = lastanim->basepic;
                    flatpicnum <= picnum; flatpicnum++)
            {
                getAddressOrCacheLumpNum(flatpicnum + _g->firstflat, true, FLASH_LEVEL_REGION);
            }

        }

        lastanim->istexture = animdefs[i].istexture;
        lastanim->numpics = picnum - lastanim->basepic + 1;

        if (lastanim->numpics < 2)
            I_Error("P_InitPicAnims: bad cycle from %s to %s", animdefs[i].startname, animdefs[i].endname);

        //_g->lastanim->speed = animdefs[i].speed; // next-hack: why wasting a byte to store the same constant value (8)?

        lastanim++;
    }
}
//
int markAnimTextures(int num, uint8_t *mustBeLoaded)
{
  int i = 0;
  int framesAdded =0;
  for (; animdefs[i].istexture != -1; i++)
  {
    if (!anims[i].istexture)
      continue;
    // is this texture within one animated texture?
    if (num >= anims[i].basepic && num < anims[i].numpics + anims[i].basepic)
    {
      // mark all textures
      for (int j = anims[i].basepic; j < anims[i].numpics + anims[i].basepic; j++)
      {
        if (!mustBeLoaded[j])
        {
          mustBeLoaded[j] = 1;
...
fixed_t P_FindLowestFloorSurrounding(sector_t *sec)
{
    ...
    const line_t *check;
    sector_t *other;
    fixed_t floor = FIXED16_TO_FIXED32(getRamSector(sec)->floorheight16);
    ...
    for (i = 0; i < sec->linecount; i++)
    {
        ...
        if (getRamSector(other)->floorheight16 < FIXED32_TO_FIXED16(floor))
            floor = FIXED16_TO_FIXED32(getRamSector(other)->floorheight16);
    }
    return floor;
}
...
fixed_t P_FindHighestFloorSurrounding(sector_t *sec)
{
    ...
    for (i = 0; i < sec->linecount; i++)
    {
        ...
        if (FIXED16_TO_FIXED32(getRamSector(other)->floorheight16) > floor)
            floor = FIXED16_TO_FIXED32(getRamSector(other)->floorheight16);
    }
    return floor;
}
...
fixed_t P_FindNextHighestFloor(sector_t *sec, int currentheight)
{
    ...
    for (i = 0; i < sec->linecount; i++)
        if ((other = getNextSector(getSectorLineByIndex(sec, i), sec)) && FIXED16_TO_FIXED32(getRamSector(other)->floorheight16) > currentheight)
        {
            int height = FIXED16_TO_FIXED32(getRamSector(other)->floorheight16);
            while (++i < sec->linecount)
                if ((other = getNextSector(getSectorLineByIndex(sec, i), sec)) && FIXED16_TO_FIXED32(getRamSector(other)->floorheight16) < height && FIXED16_TO_FIXED32(getRamSector(other)->floorheight16) > currentheight)
                    height = FIXED16_TO_FIXED32(getRamSector(other)->floorheight16);
            return height;
        }
...
fixed_t P_FindNextLowestFloor(sector_t *sec, int currentheight)
{
    ...
    for (i = 0; i < sec->linecount; i++)
        if ((other = getNextSector(getSectorLineByIndex(sec, i), sec)) && FIXED16_TO_FIXED32(getRamSector(other)->floorheight16) < currentheight)
        {
            int height = FIXED16_TO_FIXED32(getRamSector(other)->floorheight16);
            while (++i < sec->linecount)
                if ((other = getNextSector(getSectorLineByIndex(sec, i), sec)) && FIXED16_TO_FIXED32(getRamSector(other)->floorheight16) > height 
                    && FIXED16_TO_FIXED32(getRamSector(other)->floorheight16) < currentheight)
                    height = FIXED16_TO_FIXED32(getRamSector(other)->floorheight16);
            return height;
        }
    return currentheight;
...
fixed_t P_FindNextLowestCeiling(sector_t *sec, int currentheight)
{
    ...
    for (i = 0; i < sec->linecount; i++)
        if ((other = getNextSector(getSectorLineByIndex(sec, i), sec)) && FIXED16_TO_FIXED32(getRamSector(other)->ceilingheight16) < currentheight)
        {
            int height = FIXED16_TO_FIXED32(getRamSector(other)->ceilingheight16);
            while (++i < sec->linecount)
                if ((other = getNextSector(getSectorLineByIndex(sec, i), sec)) 
                    && FIXED16_TO_FIXED32(getRamSector(other)->ceilingheight16) > height 
                    && FIXED16_TO_FIXED32(getRamSector(other)->ceilingheight16) < currentheight)
                    height = FIXED16_TO_FIXED32(getRamSector(other)->ceilingheight16);
            return height;
        }
    return currentheight;
}
...
fixed_t P_FindLowestCeilingSurrounding(sector_t *sec)
{
    ...
    for (i = 0; i < sec->linecount; i++)
    {
        ...
        if (FIXED16_TO_FIXED32(getRamSector(other)->ceilingheight16) < height)
            height = FIXED16_TO_FIXED32(getRamSector(other)->ceilingheight16);
    }
    return height;
}
    ...
    for (i = 0; i < sec->linecount; i++)
    {
        if (twoSided(secnum, i))
        {
            ...
            if (side->toptexture > 0)   //jff 8/14/98 texture 0 is a placeholder
                if (getTextureHeight(side->toptexture) < minsize)
                    minsize = getTextureHeight(side->toptexture);
            side = getSide(secnum, i, 1);
            if (side->toptexture > 0)   //jff 8/14/98 texture 0 is a placeholder
                if (getTextureHeight(side->toptexture) < minsize)
                    minsize = getTextureHeight(side->toptexture);
...
sector_t* P_FindModelFloorSector(fixed_t floordestheight, int secnum)
{
    ...
    for (i = 0; i < (linecount); i++)
    {
        if (twoSided(secnum, i))
        {
            ...
            if (FIXED16_TO_FIXED32(getRamSector(sec)->floorheight16) == floordestheight)
                return sec;
        }
    }
    return NULL;
...
sector_t* P_FindModelCeilingSector(fixed_t ceildestheight, int secnum)
{
    ...
    for (i = 0; i < (linecount); i++)
    {
        if (twoSided(secnum, i))
        {
            ...
            if (getRamSector(sec)->ceilingheight16 == FIXED32_TO_FIXED16(ceildestheight))
                return sec;
        }
    }
    return NULL;
...
int P_FindMinSurroundingLight(sector_t *sector, int max)
{
    ...
    for (i = 0; i < sector->linecount; i++)
    {
        ...
        if (getRamSector(check)->lightlevel < min)
            min = getRamSector(check)->lightlevel;
    }
    return min;
}
...
boolean PUREFUNC P_SectorActive(special_e t, const sector_t *sec)
{
    ...
    {
        case floor_special:
            return getRamSector(sec)->floordata_sptr != 0;
        case ceiling_special:
            return getRamSector(sec)->ceilingdata_sptr != 0;
...
{
    return (getRamSector(sec)->special == 9 || (getRamSector(sec)->special & SECRET_MASK));
}
...
{
    return getRamSector(sec)->wasSecret; //(getRamSector(sec)->oldspecial == 9 || (getRamSector(sec)->oldspecial & SECRET_MASK));
}
...
void P_CrossSpecialLine(const line_t *line, int side, mobj_t *thing)
{
    ...
    if (linefunc) // if it was a valid generalized type
        switch ((LN_SPECIAL(line) & TriggerType) >> TriggerTypeShift)
        {
            case WalkOnce:
                if (linefunc(line))
                    LN_CLR_SPECIAL(line);   // clear special if a walk once type
                return;
            case WalkMany:
                linefunc(line);
                return;
    ...
    switch (LN_SPECIAL(line))
    {
        ...
        case 125:
            ...
            if (!getMobjPlayer(thing) && (EV_Teleport(line, side, thing)))
                LN_CLR_SPECIAL(line);
            break;
        ...
        default:
            switch (LN_SPECIAL(line))
            {
                ...
                case 264: //jff 4/14/98 add monster-only silent line-line reversed
                    if (!getMobjPlayer(thing) && EV_SilentLineTeleport(line, side, thing, true))
                        LN_CLR_SPECIAL(line);
                    break;

                case 266: //jff 4/14/98 add monster-only silent line-line
                    if (!getMobjPlayer(thing) && EV_SilentLineTeleport(line, side, thing, false))
                        LN_CLR_SPECIAL(line);
                    break;

                case 268: //jff 4/14/98 add monster-only silent
                    if (!getMobjPlayer(thing) && EV_SilentTeleport(line, side, thing))
                        LN_CLR_SPECIAL(line);
                    break;
...
void P_PlayerInSpecialSector(player_t *player)
{
    ...
    sector = &_g->sectors[getMobjSubsector(player->mo)->sector_num];

    // Falling, not all the way down yet?
    // Sector specials don't apply in mid-air
    if (player->mo->zr != FIXED16_TO_FIXED_Z(getRamSector(sector)->floorheight16))
        return;

    // Has hit ground.
    //jff add if to handle old vs generalized types
    if (getRamSector(sector)->special < 32) // regular sector specials
    {
        switch (getRamSector(sector)->special)
        {
            case 5:
    ...
    {
        switch ((getRamSector(sector)->special & DAMAGE_MASK) >> DAMAGE_SHIFT)
        {
            case 0: // no damage
                break;
        ...
        if (getRamSector(sector)->special & SECRET_MASK)
        {
            player->secretcount++;
            getRamSector(sector)->special &= ~SECRET_MASK;
            if (getRamSector(sector)->special < 32) // if all extended bits clear,
                getRamSector(sector)->special = 0;    // sector is not special anymore
        }
...
//
uint32_t      levelTimeCount;
uint16_t      levelFragLimitCount; // Ty 03/18/98 Added -frags support
void P_UpdateSpecials(void)
{
    ...
    if (_g->netgame)
    {
      ...
      // next-hack: dont' downcount if already 0: it means no limit.
       if (levelTimeCount)
       {
         levelTimeCount--;
         if (!levelTimeCount)
       ...
       if (levelFragLimitCount)  // we used -frags so compare count
       {
    ...
    for (anim = anims; anim < lastanim; anim++)
    {
        ...
        {
            if (anim->istexture)
                texturetranslation[i] = deltapic;
            else
                flattranslation[i] = deltapic;
            deltapic--;
        }
    }
...
void P_SpawnSpecials(void)
{
    ...
    //
#if HAS_NETWORK
    levelTimeCount = M_GeTimeLimit() * 60 * TICRATE;
    levelFragLimitCount = M_GetFragLimit(); // Ty 03/18/98 Added -frags support
#endif
    //  Init special sectors.
    sector = _g->sectors;
    for (i = 0; i < _g->numsectors; i++, sector++)
    {
        if (!getRamSector(sector)->special)
            continue;

        if (getRamSector(sector)->special & SECRET_MASK) //jff 3/15/98 count extended
            _g->totalsecret++;                 // secret sectors too

        switch (getRamSector(sector)->special & 31)
        {
            case 1:
                // random off
            ...
            case 4:
                ...
                P_SpawnStrobeFlash(sector, FASTDARK, 0);
                getRamSector(sector)->special |= 3 << DAMAGE_SHIFT; //jff 3/14/98 put damage bits in
                break;
            ...
            case 9:
                // secret sector
                if (getRamSector(sector)->special < 32) //jff 3/14/98 bits don't count unless not
...
void THINKFUN_ALIGN4 T_Scroll(scroll_t *s)
{
...
static void Add_Scroller(int affectee)
{
    ...
    s->thinker.function_idx = THINKER_IDX(T_Scroll);
    s->affectee = affectee;
