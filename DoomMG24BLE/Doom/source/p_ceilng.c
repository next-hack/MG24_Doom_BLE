 ...
 *
 *   next-hack: modified code to work with short pointers, and 16-bit
 *   z coordinates
 *
 *-----------------------------------------------------------------------------*/
...
//
void THINKFUN_ALIGN4 T_MoveCeiling(ceiling_t *ceiling)
{
    result_e res;
        ...
        case 1:
            ...
            if (res == pastdest)
            {
                switch (ceiling->type)
                {
                    ...
                    case genCeilingChgT:
                    case genCeilingChg0:
                        getRamSector(ceiling->sector)->special = ceiling->newspecial;
                        //jff 3/14/98 transfer old special field as well
                        getRamSector(ceiling->sector)->wasSecret = ceiling->oldspecial;
                        // fall through
                    case genCeilingChg:
...
int EV_DoCeiling(const line_t *line, ceiling_e type)
{
    ...
    while ((secnum = P_FindSectorFromLineTag(line, secnum)) >= 0)
    {
        ...
        switch (type)
        {
            ...
            case lowerAndCrush:
            case lowerToFloor:
                ceiling->bottomheight = FIXED16_TO_FIXED32(getRamSector(sec)->floorheight16);
...
int EV_CeilingCrushStop(const line_t *line)
{
    ...
    for (cl = _g->activeceilings; cl; cl = cl->next)
    {
        ...
        if (ceiling->direction != 0 && ceiling->tag == line->tag)
        {
            ...
            ceiling->direction = 0;
            ceiling->thinker.function_idx = 0;
            rtn = 1;
        }
    }
    return rtn;
...
void P_RemoveActiveCeiling(ceiling_t *ceiling)
{
    ...
    getRamSector(ceiling->sector)->ceilingdata_sptr = 0;  //jff 2/22/98
