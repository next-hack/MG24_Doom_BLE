 ...
 *
 *  next-hack: modified code to work with short pointers, optimized a lot memory,
 *  and used 16-bit z coords.
 *
 *-----------------------------------------------------------------------------*/
        ...
        switch (Targ)
        {
            ...
            case FtoNnF:
                floor->floordestheight =
                        Dirn ? P_FindNextHighestFloor(sec, FIXED16_TO_FIXED32(getRamSector(sec)->floorheight16)) : P_FindNextLowestFloor(sec, FIXED16_TO_FIXED32(getRamSector(sec)->floorheight16));
                break;
            case FtoLnC:
            ...
            case FtoC:
                ...
                break;
            case FbyST:
                floor->floordestheight = (FIXED16_TO_FIXED32(getRamSector(getFloorMoveSector(floor))->floorheight16) >> FRACBITS) + floor->direction * (P_FindShortestTextureAround(secnum) >> FRACBITS);
 #warning to check if floor height can even reach such value
                ...
                break;
            case Fby24:
                floor->floordestheight = FIXED16_TO_FIXED32(getRamSector(getFloorMoveSector(floor))->floorheight16)+ floor->direction * 24 * FRACUNIT;
                break;
            case Fby32:
                floor->floordestheight = FIXED16_TO_FIXED32(getRamSector(getFloorMoveSector(floor))->floorheight16) + floor->direction * 32 * FRACUNIT;
                break;
            default:
                break;
        }
        ...
        if (ChgT)   // if a texture change is indicated
        {
            ...
            {
                floor->texture = getRamSector(LN_FRONTSECTOR(line))->floorpic;
                switch (ChgT)
                {
                    case FChgZero:    // zero type
                        floor->newspecial = 0;
                        ...
                        break;
                    case FChgTyp:     // copy type
                        floor->newspecial = getRamSector(LN_FRONTSECTOR(line))->special;
                        //jff 3/14/98 change old field too
                        floor->oldspecial = getRamSector(LN_FRONTSECTOR(line))->wasSecret;
                        floor->type = genFloorChgT;
                        break;
                    case FChgTxt:     // leave type be
                        floor->type = genFloorChg;
                        break;    // 2023-03-12 next-hack: remove warning.
                    default:
                        break;
                }
            }
...
int EV_DoGenCeiling(const line_t *line)
{
    ...
    while ((secnum = P_FindSectorFromLineTag(line, secnum)) >= 0)
    {
        ...
        getRamSector(sec)->ceilingdata_sptr = getShortPtr(ceiling); //jff 2/22/98
        ceiling->thinker.function_idx = THINKER_IDX(T_MoveCeiling);
        ceiling->crush = Crsh;
        ceiling->direction = Dirn ? 1 : -1;
        ceiling->sector = sec;
        ceiling->texture = sec->ceilingpic;
        ceiling->newspecial = getRamSector(sec)->special;
        //jff 3/14/98 change old field too
        ceiling->oldspecial = getRamSector(sec)->wasSecret;
        ceiling->tag = sec->tag;
        ...
        targheight = FIXED16_TO_FIXED32(getRamSector(sec)->ceilingheight16);
        switch (Targ)
        {
            case CtoHnC:
            ...
            case CtoNnC:
                targheight =
                        Dirn ? P_FindNextHighestCeiling(sec, FIXED16_TO_FIXED32(getRamSector(sec)->ceilingheight16)) : P_FindNextLowestCeiling(sec, FIXED16_TO_FIXED32(getRamSector(sec)->ceilingheight16));
                break;
            case CtoHnF:
                targheight = P_FindHighestFloorSurrounding(sec);
                break;
            case CtoF:
                targheight = FIXED16_TO_FIXED32(getRamSector(sec)->floorheight16);
                break;
            case CbyST:
                targheight = (FIXED16_TO_FIXED32(getRamSector(ceiling->sector)->ceilingheight16) >> FRACBITS) + ceiling->direction * (P_FindShortestUpperAround(secnum) >> FRACBITS);
      #warning FIXME. Overflow is now musch smaller
                ...
                break;
            case Cby24:
                targheight = FIXED16_TO_FIXED32(getRamSector(ceiling->sector)->ceilingheight16) + ceiling->direction * 24 * FRACUNIT;
                break;
            case Cby32:
                targheight = FIXED16_TO_FIXED32(getRamSector(ceiling->sector)->ceilingheight16) + ceiling->direction * 32 * FRACUNIT;
                break;
            default:
                break;
        }
            ...
            else        // else if a trigger model change
            {
                ...
                switch (ChgT)
                {
                    case CChgZero:    // type is zeroed
                        ...
                        break;
                    case CChgTyp:     // type is copied
                        ceiling->newspecial = getRamSector(LN_FRONTSECTOR(line))->special;
                        //jff 3/14/98 change old field too
                        ceiling->oldspecial = getRamSector(LN_FRONTSECTOR(line))->wasSecret;
                        ceiling->type = genCeilingChgT;
                        break;
                    case CChgTxt:     // type is left alone
                        ceiling->type = genCeilingChg;
...
int EV_DoGenLift(const line_t *line)
{
    ...
    while ((secnum = P_FindSectorFromLineTag(line, secnum)) >= 0)
    {
        ...
        plat->sector = sec;
        getRamSector(plat->sector)->floordata_sptr = getShortPtr(plat);
        plat->thinker.function_idx = THINKER_IDX(T_PlatRaise);
        plat->crush = false;
        plat->tag = line->tag;

        plat->type = genLift;
        plat->high = FIXED16_TO_FIXED32(getRamSector(sec)->floorheight16);
        plat->status = down;
        ...
        switch (Targ)
        {
            case F2LnF:
                ...
                if (plat->low > FIXED16_TO_FIXED32(getRamSector(sec)->floorheight16))
                    plat->low = FIXED16_TO_FIXED32(getRamSector(sec)->floorheight16);
                break;
            case F2NnF:
                plat->low = P_FindNextLowestFloor(sec, FIXED16_TO_FIXED32(getRamSector(sec)->floorheight16));
                break;
            case F2LnC:
            ...
            case LnF2HnF:
                ...
                if (plat->high < FIXED16_TO_FIXED32(getRamSector(sec)->floorheight16))
                    plat->high = FIXED16_TO_FIXED32(getRamSector(sec)->floorheight16);
...
int EV_DoGenStairs(const line_t *line)
{
    ...
    int Dirn = LN_SPECIAL_STAIR_DIRECTION(line) >> StairDirectionShift;//(value & StairDirection) >> StairDirectionShift;
    ...
    while ((secnum = P_FindSectorFromLineTag(line, secnum)) >= 0)
    {
        ...
        speed = floor->speed;
        height = FIXED16_TO_FIXED32(getRamSector(sec)->floorheight16) + floor->direction * stairsize;
        ...
        do
        {
            ...
            for (i = 0; i < sec->linecount; i++)
            {
                ...
                if (!Igno && getRamSector(tsec)->floorpic != texture)
                    continue;
    ...
    if (rtn)
       /*LN_SPECIAL(line) ^= StairDirection;*/
       LN_TOGGLE_SPECIAL_STAIR_DIRECTION(line); // alternate dir on succ activations
    return rtn;
}
...
int EV_DoGenCrusher(const line_t *line)
{
    ...
    while ((secnum = P_FindSectorFromLineTag(line, secnum)) >= 0)
    {
        ...
        ceiling->topheight = FIXED16_TO_FIXED32(getRamSector(sec)->ceilingheight16);
        ceiling->bottomheight = FIXED16_TO_FIXED32(getRamSector(sec)->floorheight16) + (8 * FRACUNIT);
...
int EV_DoGenLockedDoor(const line_t *line)
{
    ...
    while ((secnum = P_FindSectorFromLineTag(line, secnum)) >= 0)
    {
        ...
        getRamSector(sec)->ceilingdata_sptr = getShortPtr(door); //jff 2/22/98

        door->thinker.function_idx = THINKER_IDX(T_VerticalDoor);
        door->sector = sec;
    ...
    while ((secnum = P_FindSectorFromLineTag(line, secnum)) >= 0)
    {
        ...
        switch (Kind)
        {
            case OdCDoor:
                ...
                door->topheight = P_FindLowestCeilingSurrounding(sec);
                door->topheight -= 4 * FRACUNIT;
                if (door->topheight != FIXED16_TO_FIXED32(getRamSector(sec)->ceilingheight16))
            ...
            case ODoor:
                ...
                break;
            case CdODoor:
                door->topheight = FIXED16_TO_FIXED32(getRamSector(sec)->ceilingheight16);
                door->direction = -1;
                S_StartSound2(&door->sector->soundorg,
