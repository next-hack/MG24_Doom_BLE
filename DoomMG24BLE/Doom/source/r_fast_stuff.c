 ...
 *     - Adapted to 240x240 and 320 x 200 or 240 display
...
#define CURRENT_COLOR_MAP_IDX 61
#define FIXED_COLOR_MAP_IDX 62
#define SHADOW_COLOR_MAP_IDX 63
...
__attribute__ ((section(".openings_bss"))) maskedData_t maskedData;
...
#define MIN_PIXEL_TO_DRAW_FOR_CACHING_COLORMAP 8 // 16
#if !CACHE_ALL_COLORMAP_TO_RAM
__attribute__ ((aligned(4))) static uint8_t colorMap[256];  // for faster drawing
static uint8_t *currentCachedColorMap = NULL;
#endif
typedef struct
{
    byte padding[EXT_MEMORY_HEADER_SIZE];
    byte buffer[MAX_COLUMN_DATA + EXT_MEMORY_READ_ALIGN_SIZE];  // column not aligned require 3 more bytes. LDMA requires alignment based on data size
} columnBuffer_t;
static columnBuffer_t columnBuffer[2];
static columnBuffer_t columnBotBuffer[2];

//
patch_t * cachedColumnOffsetDataPatch = NULL;
static uint32_t columnOffsetData[MAX_PATCH_WIDTH];
#define CHECK_ALIGNMENT(m) do { if ((uint32_t) (m) & 3) {printf("Unaligned address %x at line %d", m, __LINE__); while(1);}}while(0)
...
static byte *solidcol; // next-hack: put in stack. [MAX_SCREENWIDTH];
...
//fixed_t *textureheight; //needed for texture pegging (and TFE fix - killough) // 2022-02-01 next-hack removed.
// next-hack: flat and texture translation are now offset to the original texture index.
int8_t *flattranslation;             // for global animation
int8_t *texturetranslation;
...
static const angle_t clipangle = 537395200; //xtoviewangle[0]; Should always be the same regardless resolution.

static const int skytexturemid = (200 / 2 - 12) * FRACUNIT;
static const fixed_t skyiscale = (FRACUNIT * 200) / (SCREENHEIGHT) ;
...
#if !CACHE_ALL_COLORMAP_TO_RAM
static void  colormapCopy(void *dst, const void *src)
{
  register void *r0 __asm("r0") = dst;
  register const void *r1 __asm("r1") = src;
  __asm volatile
  (
...
static inline void* byteShortPtrToAddr(int16_t shortPtr)
{
  // note: sum, not OR. And signed, because of how openings work...
    if (shortPtr)
        return (void*) ((int32_t) &maskedData +  shortPtr);
    else
        return NULL;
}
...
FASTFUN inline static void R_DrawTranslatedColumnPixel(pixel *dest, const byte *source, const byte *colormap, unsigned int frac, const byte *translation)
{
    *dest = colormap[translation[source[frac >> COLBITS]]];
}
...
// 2023/03/12 next-hack: done R_DrawTranslatedColumn
FASTFUN static void R_DrawTranslatedColumn(const draw_column_vars_t *dcvars)
{
    //
    ...
    {
        colormapCopy(colorMap, dcvars->colormap);
    ...
    while (l--)
    {
        R_DrawTranslatedColumnPixel((pixel*) dest, source, colormap, frac, translation);
        dest += SCREENWIDTH;
        frac += fracstep;
        R_DrawTranslatedColumnPixel((pixel*) dest, source, colormap, frac, translation);
        dest += SCREENWIDTH;
    ...
    switch (r)
    {
        case 15:
            R_DrawTranslatedColumnPixel((pixel*) dest, source, colormap, frac, translation);
            dest += SCREENWIDTH;
            frac += fracstep; // @suppress("No break at end of case")
            // fall through
        case 14:
            R_DrawTranslatedColumnPixel((pixel*) dest, source, colormap, frac, translation);
            dest += SCREENWIDTH;
...
FASTFUN static void R_DrawColumn(const draw_column_vars_t *dcvars)
{
#if 0
...
#define BETTER_ASM_CODE 1
 ...
 while (l--)
 {
   uint32_t dummyTmp = dummyTmp;      // this to avoid uninit variable warning

#if SCREENWIDTH > 255
#define DRAW_EVEN_PIXEL    "lsrs %[dummyTmp], %[frac], %[fracShift]\n\t"      \
                           "add %[frac], %[fracStep]\n\t"                     \
                           "ldrb %[dummyTmp], [%[source], %[dummyTmp]]\n\t"   \
                           "ldrb %[dummyTmp], [%[colormap], %[dummyTmp]]\n\t" \
                           "strb %[dummyTmp], [%[dest]], %[screenwidth] / 2\n\t"
#define DRAW_ODD_PIXEL    "lsrs %[dummyTmp], %[frac], %[fracShift]\n\t"                \
                           "add %[frac], %[fracStep]\n\t"                     \
                           "ldrb %[dummyTmp], [%[source], %[dummyTmp]]\n\t"   \
                           "ldrb %[dummyTmp], [%[colormap], %[dummyTmp]]\n\t" \
                           "strb %[dummyTmp], [%[dest], %[screenwidth] / 2]!\n\t"
#define INCREMENT_DEST_VALUE    "add %[dest], %[screenwidth]\n\t"
#define DRAW_TWO_COLUMN_PIXEL DRAW_EVEN_PIXEL DRAW_ODD_PIXEL
...
#define DRAW_EVEN_PIXEL    "lsrs %[dummyTmp], %[frac], %[fracShift]\n\t"               \
                           ...
                           "ldrb %[dummyTmp], [%[source], %[dummyTmp]]\n\t"   \
                           "ldrb %[dummyTmp], [%[colormap], %[dummyTmp]]\n\t" \
                           "strb %[dummyTmp], [%[dest]], %[screenwidth]\n\t"
#define DRAW_ODD_PIXEL     DRAW_EVEN_PIXEL
#define INCREMENT_DEST_VALUE
#endif
   __asm volatile
   (
      ".align(2)\n\t"
      DRAW_TWO_COLUMN_PIXEL
      INCREMENT_DEST_VALUE
      DRAW_TWO_COLUMN_PIXEL
      INCREMENT_DEST_VALUE
      DRAW_TWO_COLUMN_PIXEL
      INCREMENT_DEST_VALUE
...
static inline uint32_t getColumnOffsetData(patch_t *patch, int colindex)
{
    ...
    {
      uint32_t coloffs;
      extMemSetCurrentAddress(((uint32_t) patch + 8 + 4 * colindex));
      extMemGetDataFromCurrentAddress(&coloffs, 4);
      return coloffs;
    }
}
// this will get a
static inline uint8_t *  getColumnDataNonBlocking(const patch_t *patch, int colindex, columnBuffer_t *columnData)
{
    uint32_t co = getColumnOffsetData((patch_t*) patch, colindex);
    uint32_t cnt = co >> 24;
    co = co & 0xFFFFFF;
    uint32_t addr = (uint32_t) patch + co;
    return extMemStartAsynchDataRead(addr, columnData, cnt);
}
FASTFUN static inline column_t* getColumnData(const patch_t *patch, int colindex, columnBuffer_t *columnData)
{
// Note: this is only on sky, but it should not be on external flash!
    void *readAddress;
    readAddress = getColumnDataNonBlocking(patch, colindex, columnData);
    extMemWaitAsynchDataRead();
    return (column_t*) readAddress;
}
static inline void cachePatchColumnOffsetData(patch_t *patch, int width, int startX, int stopX)
{
    ...
    if ( 260 * cnt > 30 + width * 16)  
    {
        ...
        extMemSetCurrentAddress(((uint32_t) patch + 8));
        extMemGetDataFromCurrentAddress(columnOffsetData, 4 * width);
    }
}
...
FASTFUN static void R_DrawVisSprite(const vissprite_t *vis)
{
    ...
    {
        // 2023/03/12 next-hack: restored translation. However vissprites do not store full mobjflags
        //                       but only the 2 bits corresponding the MF_TRANSLATION
        if (vis->mobjflags)
        {
          colfunc = R_DrawTranslatedColumn;
          dcvars.translation = translationtables + ((vis->mobjflags - 1) << 8);
        }
        else
        {
...
//        uint32_t oldColindex = 0xFFFFFFFF;  // invalid index
        ...
        cachePatchColumnOffsetData((patch_t*) patch, p_wad_immutable_flash_data->patchLumpSizeOffsets[vis->lumpNum - _g->firstPatchLumpNum].width, vis->x1 , vis->x2);
        column[bufferNumber] = getColumnDataNonBlocking(patch, colindex, &columnBuffer[bufferNumber]);
        ...
        if (absXiscale >= 0x10000)
        {
            for (dcvars.x = vis->x1; dcvars.x < vis->x2;  dcvars.x++, frac += vis->xiscale)
            {
                ...
                column[1 - bufferNumber] = getColumnDataNonBlocking(patch, colindex, &columnBuffer[1 -bufferNumber]);
                R_DrawMaskedColumn(colfunc, &dcvars, (column_t*) column[bufferNumber]);
                // swap buffers
...
{
    // textures width are power of two
    const unsigned int widthmask = texture->width - 1; //texture->widthmask;
    ...
    {
        return getColumnData(patch, xc, (columnBuffer_t *) columnData);
    }
}
...
FASTFUN static inline short* openingSsptrToAddr(unsigned short ssptr)
{
   if (ssptr <= SCREENHEIGHTARRAY_PTR)
    ...
    {
        return (short*) ((uint32_t) &maskedData + (ssptr));
...
/*  if ((uint32_t) ptr & 0x20001)
  {
    ...
    return ((int) ptr - (int32_t) &maskedData);
...
FASTFUN static void R_RenderMaskedSegRange(const drawseg_t *ds, int x1, int x2)
{
    ...
    //curline = ds->curline; 
    curline = &_g->segs[ds->curlineindex];  // OPTIMIZE: get rid of LIGHTSEGSHIFT globally
    ...
    texnum += texturetranslation[texnum];  // next-hack: was texnum = texturetranslation[texnum]; without +. 
    // next-hack: added back lighting

    int lightnum = (getRamSector(frontsector)->lightlevel >> LIGHTSEGSHIFT) + extralight;
    ...
    rw_lightlevel = getRamSector(frontsector)->lightlevel;

    //maskedData.maskedtexturecol = ds->maskedtexturecol; //&openings[ds->maskedtexturecolindex];
    maskedData.maskedtexturecol = byteShortPtrToAddr(ds->maskedtexturecol_sptrb);//openingSsptrToAddr(ds->maskedtexturecol_sptrb); //byteShortPtrToAddr(ds->maskedtexturecol_sptrb);
    rw_scalestep = (ds->scale2 - ds->scale1) / (ds->x2 - ds->x1);  // ds->scalestep;
    ...
    {
        dcvars.texturemid =
                getRamSector(frontsector)->floorheight16 > getRamSector(backsector)->floorheight16 ? FIXED16_TO_FIXED32(getRamSector(frontsector)->floorheight16) : FIXED16_TO_FIXED32(getRamSector(backsector)->floorheight16);
        dcvars.texturemid = dcvars.texturemid + getTextureHeight(texnum) - viewz;
    }
    else
    {
        dcvars.texturemid =
                getRamSector(frontsector)->ceilingheight16 < getRamSector(backsector)->ceilingheight16 ? FIXED16_TO_FIXED32(getRamSector(frontsector)->ceilingheight16) : FIXED16_TO_FIXED32(getRamSector(backsector)->ceilingheight16);
    ...
    const patch_t *patch = texture->patches[0].patch;
    // textures width are power of two
    const unsigned int widthmask = texture->width - 1;
    if (isOnExternalFlash(patch))
    {
        if (x1 <= x2)       // is there any column to draw?
        {
            // if the texture is in flash, then we draw it asynchronously: when we load a column via DMA, we draw the previous one
            int x = x1;
            uint32_t bufferNumber = 0;
            int xc = maskedData.maskedtexturecol[x];
            // get the first column
            cachePatchColumnOffsetData( (patch_t*) texture->patches[0].patch, texture->width, x1 , x2);
            //
            column = (column_t*) getColumnDataNonBlocking(patch, xc & widthmask, &columnBuffer[bufferNumber]);
            //
            for (dcvars.x = x1; dcvars.x < x2; dcvars.x++, spryscale += rw_scalestep)
            {
                if (xc != D_SHRT_MAX) // dropoff overflow
                {
                    ...
                    // draw the texture
                    extMemWaitAsynchDataRead();
                    nextColumn = (column_t*) getColumnDataNonBlocking(patch, xc & widthmask, &columnBuffer[1 - bufferNumber]);
                    R_DrawMaskedColumn(R_DrawColumn, &dcvars, column);
                    column = nextColumn;
                    maskedData.maskedtexturecol[dcvars.x] = D_SHRT_MAX; // dropoff overflow
                    bufferNumber = 1 - bufferNumber;
                }
                xc = maskedData.maskedtexturecol[dcvars.x + 1];
            }
            // draw last column
            if (xc != D_SHRT_MAX) // dropoff overflow
            {
                ...
                //
                dcvars.iscale = FixedReciprocal((unsigned) spryscale);
                // draw the texture. However, wait till the buffer is ready, as we read if from external flash.
                extMemWaitAsynchDataRead();
                R_DrawMaskedColumn(R_DrawColumn, &dcvars, column);
                maskedData.maskedtexturecol[dcvars.x] = D_SHRT_MAX; // dropoff overflow
            }
    ...
    {
      // texture in internal flash
      for (dcvars.x = x1; dcvars.x <= x2; dcvars.x++, spryscale += rw_scalestep)
      {
          const int xc = maskedData.maskedtexturecol[dcvars.x];
          ...
          if (xc != D_SHRT_MAX) // dropoff overflow
          {
              ...
              column = (column_t*) ((byte*) patch + (0xFFFFFF & patch->columnofs[xc & widthmask]));
...
{
    ycoord_t *clipbot = maskedData.floorclip;
    ycoord_t *cliptop = maskedData.ceilingclip;
    ...
    for (const drawseg_t *ds = ds_p; ds-- > drawsegs;)  // new -- killough
    {
        ...
        if (ds->x1 > spr->x2 || ds->x2 < spr->x1 || (!ds->silhouette && ds->maskedtexturecol_sptrb == 0))
        ...
        mobj_t *pmobj = (mobj_t*) getLongPtr(spr->mobj_sptr);

        if (scale < spr->scale || (lowscale < spr->scale && !R_PointOnSegSide(getMobjX(pmobj), getMobjY(pmobj), &_g->segs[ds->curlineindex])))
        {
            if (ds->maskedtexturecol_sptrb)    // masked mid texture?
        ...
        fixed16_t gz16 = FIXED32_TO_FIXED16(getMobjZ(pmobj));
        if ((ds->silhouette & SIL_BOTTOM) && gz16 < ds->bsilheight16) //bottom sil
        {
            for (int x = r1; x <= r2; x++)
            {
        ...
        patchsizeoffsets_t *patch = &p_wad_immutable_flash_data->patchLumpSizeOffsets[spr->lumpNum - _g->firstPatchLumpNum];
        fixed16_t gzt16 = gz16 + FIXED32_TO_FIXED16(patch->topoffset << FRACBITS);
        if ((ds->silhouette & SIL_TOP) && gzt16 > ds->tsilheight16)  // top sil
        {
            for (int x = r1; x <= r2; x++)
            {
...
FASTFUN static void R_DrawPSprite(pspdef_t *psp)
{
    int x1, x2;
    spritedef_t *sprdef;
    ...
    patch = &p_wad_immutable_flash_data->patchLumpSizeOffsets[lumpNum - _g->firstPatchLumpNum];
...
{
    int i;
    int lightlevel = _g->ramsectors[getMobjSubsector(_g->viewplayer->mo)->sector_num].lightlevel;
    pspdef_t *psp;
    ...
    mfloorclip = maskedData.floorclip;
    mceilingclip = maskedData.ceilingclip;
    mfloorceiling_ptr_size = sizeof(*maskedData.floorclip);
 ...
 #else
    uint32_t *p_fc = (uint32_t*) maskedData.floorclip;
    uint32_t *p_cc = (uint32_t*) maskedData.ceilingclip;
...
FASTFUN static inline void R_DrawSpan(unsigned int y, unsigned int x1, unsigned int x2, const draw_span_vars_t *dsvars)
{
    ...
    {
        colormapCopy(colorMap, dsvars->colormap);
    ...
    switch (r)
    {
        case 15:
            R_DrawSpanPixel((pixel*) dest, source, colormap, position);
            dest += 1;
            position += step; // @suppress("No break at end of case")
            // fall through
        case 14:
            R_DrawSpanPixel((pixel*) dest, source, colormap, position);
            dest += 1;
...
FASTFUN static void R_DoDrawPlane(visplane_t *pl)
{
    ...
    if (pl->minx <= getVisplaneMaxx(pl))
    {
        ...
        { // sky flat
            ...
            cachePatchColumnOffsetData((patch_t *) tex->patches[0].patch, tex->width,  pl->minx, getVisplaneMaxx(pl));
            ...
            for (x = pl->minx; (dcvars.x = x) <= getVisplaneMaxx(pl); x++)
            {
                if ((dcvars.yl = pl->top[x]) != -1 && dcvars.yl <= (dcvars.yh = pl->bottom[x])) // dropoff overflow
                {
                    ...
                    {
                        oldXc = xc;
                        column = R_GetColumn(tex, xc, (uint8_t *) columnBuffer);
                    }
                    dcvars.source = (const byte*) column + 3;
                    R_DrawColumn(&dcvars);
                }
        ...
        {     // regular flat
            ...
            uint32_t light; // must be unsigned.
            dsvars.source = W_CacheLumpNum(_g->firstflat + flattranslation[pl->picnum] + pl->picnum);   // next-hack: now flats translation are just offset to the original picture index.
            ...
            planeheight = D_abs(FIXED16_TO_FIXED32(pl->height16) - viewz);
            ...
            const int stop = getVisplaneMaxx(pl) + 1;
            int minx = pl->minx;
            pl->top[minx - 1] = pl->top[stop] = 0xff; // dropoff overflow
            for (x = minx; x <= stop; x++)
            {
                int t1 = pl->top[x - 1];
...
// proff 11/06/98: Changed for high-res
    ...
    return den > num >> 16 ?
            (num = FixedApproxDiv(num, den)) > 64 * FRACUNIT ? 64 * FRACUNIT :
...
{
    const fixed_t fx = getMobjX(thing);
    const fixed_t fy = getMobjY(thing);
    const fixed_t fz = FIXED_Z_TO_FIXED32(thing->zr);
    ...
    const spritedef_t *sprdef = &p_wad_immutable_flash_data->sprites[getMobjSprite(thing)];
    const spriteframe_t *sprframe = (const spriteframe_t*) &(getSpriteFrames(sprdef)[getMobjFrame(thing) & FF_FRAMEMASK]);
    ...
    if (sprframe->rotate)
    {
        ...
        rot = (ang - ANGLE16_TO_ANGLE32(thing->angle16) + (unsigned) (ANG45 / 2) * 9) >> 29;
    }
    ...
    const fixed_t xscale = FixedApproxDiv(projection, tz);
...
// 2021-02-13 next-hack: useless having to store 32 bit, when you need only 2...
    vis->mobjflags = 0;
    // 2023-03-12 next-hack: supported back multiplayer and translation.
    for(int n = 1; n < MAXPLAYERS; n++)
    {
      if (thing->type == MT_PLAYER)
      {
        vis->mobjflags = thing->playerCorpse_n;
        break;
      }
    }
    // proff 11/06/98: Changed for high-res
    vis->scale = FixedApproxDiv(projectiony, tz);
    vis->lumpNum = lumpNum;
// next-hack: removed these. If we store the short pointer.
//    vis->gx = fx;
//    vis->gy = fy;
//    vis->gz16 = FIXED32_TO_FIXED16(fz);
    vis->mobj_sptr = getShortPtr(thing);
    //
    //vis->texturemid = (FIXED16_TO_FIXED32(vis->gz16) + (patch->topoffset << FRACBITS)) - viewz;
    vis->texturemid = (fz + (patch->topoffset << FRACBITS)) - viewz;
    vis->x1 = x1 < 0 ? 0 : x1;
    vis->x2 = x2 >= SCREENWIDTH ? SCREENWIDTH - 1 : x2;

    const fixed_t iscale = FixedApproxDiv(FRACUNIT, xscale);
    ...
    // get light level
    if (getMobjFlags(thing) & MF_SHADOW)
        vis->colormap_idx = SHADOW_COLOR_MAP_IDX;             // shadow draw
    else if (fixedcolormap)
        vis->colormap_idx = FIXED_COLOR_MAP_IDX;      // fixed map
    else if (getMobjFrame(thing) & FF_FULLBRIGHT)
        vis->colormap_idx = FULL_COLOR_MAP_IDX; // full bright  // killough 3/20/98
    else
    {      // diminished light
...
{
    sector_t *sec = &_g->sectors[subsec->sector_num];
    mobj_t *thing;
...
#if OLD_VALIDCOUNT
    if (getRamSector(sec)->validcount == _g->validcount)
        return;

    // Well, now it will be done.
    getRamSector(sec)->validcount = _g->validcount;
#else
    if (getBitMask32(_g->lineSectorChecked, sec->sectorNumber))
        return;             
...
{
    visplane_t *check = getLongPtr(_g->freetail_sptr);

    if (!check)
    {
        _g->allocatedVisplanes++;
        check = Z_Calloc(1, sizeof(visplane_t), PU_LEVEL, NULL);
    //    printf("NewVispl. %d\r\n", _g->allocatedVisplanes);
    }
    else
    {
        if (!(_g->freetail_sptr = ((visplane_t *)getLongPtr(_g->freetail_sptr))->next_sptr))
            _g->freehead_psptr = &_g->freetail_sptr;
    }

    check->next_sptr = _g->visplanes_sptr[hash];
    _g->visplanes_sptr[hash] = getShortPtr(check);

    return check;
}
// next-hack: height 16 bit
static visplane_t* R_FindPlane(fixed16_t height16, int picnum, int lightlevel)
{
    visplane_t *check;
    unsigned hash;                      // killough

    if (picnum == _g->skyflatnum)
        height16 = lightlevel = 0;    // killough 7/19/98: most skies map together

    // New visplane algorithm uses hash table -- killough
    hash = visplane_hash(picnum, lightlevel, height16);

    for (check = getLongPtr(_g->visplanes_sptr[hash]); check; check = getLongPtr(check->next_sptr))  // killough
        if (height16 == check->height16 && picnum == check->picnum && lightlevel == check->lightlevel)
            return check;

    check = new_visplane(hash);         // killough

    check->height16 = height16;
    check->picnum = picnum;
...
{
    unsigned hash = visplane_hash(pl->picnum, pl->lightlevel, pl->height16);
    visplane_t *new_pl = new_visplane(hash);

    new_pl->height16 = pl->height16;
...
static visplane_t* R_CheckPlane(visplane_t *pl, int start, int stop)
{
    ...
    if (stop > getVisplaneMaxx(pl))
        intrh = getVisplaneMaxx(pl), unionh = stop;
    else
        unionh = getVisplaneMaxx(pl), intrh = stop;
...
FASTFUN inline static void* R_GetSinglePatchTextureColumnNonBlocking(const texture_t *texture, int texcolumn,  columnBuffer_t *columnData,  dmaOperation_t *dmaOperation, uint32_t isTop)
{
    // textures width are power of two
    const unsigned int widthmask = texture->width - 1; //texture->widthmask;
    ...
    {   
        ...
        uint32_t src = (uint32_t) patch
                      ...
                      + 3;
        void *readAddress;
        readAddress = extMemStartAsynchDataRead(src, columnData, texture->height);
        if (isTop)
        {
...
static inline bool isPlaneModified(visplane_t *pl)
{
// next-hack: 
    #if USE_VISPLANE_MODIFIED
        return pl->modified;
    #else
        return pl->top[0] != 0xFF;
    #endif
}
static inline void markPlaneModified(visplane_t *pl)
{
    #if USE_VISPLANE_MODIFIED
      pl->modified = 1;        
    #else
      // next-hack: used this instead of doomhack's "modified" field, we use this dirty hack
      if (pl->top[0] == 0xFF)
      {
          pl->top[0] = 0xFE;  // still outside screen, but will signal there visplane has been modified and should be drawn
      }
    #endif
}
...
static inline void R_RenderSegLoop(int rw_x)
{
    ...
    if (midtexture)
    {
      if (rw_x >= rw_stopx)
        return;
      void  *nextColumnDataPtr = nextColumnDataPtr; // self assignment to prevent uninit warning
      ...
      dcvars.source = R_GetSinglePatchTextureColumnNonBlocking (tex, texturecolumn, &columnBuffer[bufferNumber], &dmaOperation, 1);
      if (fixedcolormap)
          dcvars.colormap = fixedcolormap;
      for (; rw_x < rw_stopx; rw_x++)
      {
          ...
          int cc_rwx = maskedData.ceilingclip[rw_x];
          int fc_rwx = maskedData.floorclip[rw_x];
          ...
          if (markceiling)
          {
              ...
              if (top <= bottom)
              {
                  ...
                  markPlaneModified(ceilingplane);
              }
          ...
          {
            bufferNumber = 1 - bufferNumber;
            nextColumnDataPtr = R_GetSinglePatchTextureColumnNonBlocking (tex, texturecolumn, &columnBuffer[bufferNumber], &dmaOperation, 1);
          }
          R_DrawColumn(&dcvars);
          ...
          maskedData.floorclip[rw_x] = fc_rwx;
          maskedData.ceilingclip[rw_x] = cc_rwx;
    ...
    else
    {
        ...
        if (fixedcolormap)
        {
            topDcvars.colormap = fixedcolormap;
            botDcvars.colormap = fixedcolormap;
        }
        void *nextTopColumnPtr, *nextBotColumnPtr;
        //
        texture_t *toptex = toptex, *bottex = bottex; //self assignment to prevent undeclared data
        if (toptexture)
        {
...
//            cachePatchColumnOffsetData((patch_t*) bottex->patches[0].patch, bottex->width, rw_x , rw_stopx);
        ...
        for (; rw_x < rw_stopx; rw_x++)
        {
            ...
            // texturecolumn and lighting are independent of wall tiers
            lighttable_t *nextColorMap = nextColorMap;  // self assignment to prevent uninit warning
            fixed_t iscale  = iscale;     //self assignment to prevent uninit warning
            if (segtextured)
            {
               uint32_t index;
                // calculate texture offset
            ...
            // draw the wall tiers
            {
                ...
                if (toptexture)
                {
                    ...
                    if (mid >= yl)
                    {   
                        nextTopColumnPtr =  R_GetSinglePatchTextureColumnNonBlocking(toptex, texturecolumn,  &columnBuffer[nBuff], &dmaOperation, 1);
                        cc_rwx = mid;
                        if (validTop)
                        {   
                ...
                if (bottomtexture)          // bottom wall
                {
                    ...
                    if (mid <= yh)
                    {
                        nextBotColumnPtr =  R_GetSinglePatchTextureColumnNonBlocking(bottex, texturecolumn,  &columnBotBuffer[nBuff] , &dmaOperation, 0);
                        fc_rwx = mid;
                        if (validBot)
                        {   
                ...
                if (maskedtexture)
                    maskedData.maskedtexturecol[rw_x] = texturecolumn;
            }
...
{
    int pos = _g->lastopening - maskedData.openings;
...
//
FASTFUN static void R_StoreWallRange(const int start, const int stop, boolean solid) // 2022/03/16 next-hack: added is solid to avoid storing line flags
{
    fixed_t hyp;
    angle_t offsetangle;
    ...
    setBitMask32(_g->lineIsMapped, curline->linenum);
    ...
    ds_p->x2 = stop;
    ds_p->curlineindex = curlineIndex;
    ...
    worldtop = FIXED16_TO_FIXED32(getRamSector(frontsector)->ceilingheight16) - viewz;
    worldbottom = FIXED16_TO_FIXED32(getRamSector(frontsector)->floorheight16) - viewz;

    midtexture = toptexture = bottomtexture = maskedtexture = 0;
    ds_p->maskedtexturecol_sptrb = 0;
    if (!backsector)
    {
        // single sided line
        // next-hack: texturetranslation are now offsets to index.
        midtexture = texturetranslation[getSideMidTexture(&_g->lines[curline->linenum], sidedef)] + getSideMidTexture(&_g->lines[curline->linenum], sidedef);
        ...
        {         // bottom of texture at bottom
            fixed_t vtop = FIXED16_TO_FIXED32(getRamSector(frontsector)->floorheight16) + getTextureHeight(getSideMidTexture(&_g->lines[curline->linenum], sidedef));
        ...
        rw_midtexturemid += FixedMod((sidedef->rowoffset << FRACBITS), getTextureHeight(midtexture));
        ...
        ds_p->bsilheight16 =  FRAC16_MAX; //INT_MAX;
        ds_p->tsilheight16 = FRAC16_MIN; //INT_MIN;
    }
    else      // two sided line
    {
        ...
        { /* not solid - old code */
            ...
            if (getRamSector(frontsector)->floorheight16 > getRamSector(backsector)->floorheight16)
            {
                ds_p->silhouette = SIL_BOTTOM;
                ds_p->bsilheight16 = getRamSector(frontsector)->floorheight16;
            }
            else if (FIXED16_TO_FIXED32(getRamSector(backsector)->floorheight16) > viewz)
            {
                ds_p->silhouette = SIL_BOTTOM;
                ds_p->bsilheight16 = FRAC16_MAX;
            }

            if (getRamSector(frontsector)->ceilingheight16 < getRamSector(backsector)->ceilingheight16)
            {
                ds_p->silhouette |= SIL_TOP;
                ds_p->tsilheight16 = getRamSector(frontsector)->ceilingheight16;
            }
            else if (FIXED16_TO_FIXED32(getRamSector(backsector)->ceilingheight16) < viewz)
            {
                ds_p->silhouette |= SIL_TOP;
                ds_p->tsilheight16 = FRAC16_MIN;
            }
        }

        worldhigh = FIXED16_TO_FIXED32(getRamSector(backsector)->ceilingheight16) - viewz;
        worldlow = FIXED16_TO_FIXED32(getRamSector(backsector)->floorheight16) - viewz;

        // hack to allow height changes in outdoor areas
        if (frontsector->ceilingpic == _g->skyflatnum && backsector->ceilingpic == _g->skyflatnum)
            worldtop = worldhigh;

        markfloor = worldlow != worldbottom || getRamSector(backsector)->floorpic != getRamSector(frontsector)->floorpic || getRamSector(backsector)->lightlevel != getRamSector(frontsector)->lightlevel;

        markceiling = worldhigh != worldtop || backsector->ceilingpic != frontsector->ceilingpic || getRamSector(backsector)->lightlevel != getRamSector(frontsector)->lightlevel;

        if (getRamSector(backsector)->ceilingheight16 <= getRamSector(frontsector)->floorheight16 || getRamSector(backsector)->floorheight16 >= getRamSector(frontsector)->ceilingheight16)
            markceiling = markfloor = true;   // closed door

        if (worldhigh < worldtop)   // top texture
        {
            // next-hack: texturetranslation are now offsets to index.
            toptexture = texturetranslation[getSideTopTexture(linedef, sidedef)] + getSideTopTexture(linedef, sidedef);
            rw_toptexturemid =
                    linedef->flags & ML_DONTPEGTOP ? worldtop : FIXED16_TO_FIXED32(getRamSector(backsector)->ceilingheight16) + getTextureHeight(getSideTopTexture(linedef, sidedef)) - viewz;
            rw_toptexturemid += FixedMod((sidedef->rowoffset << FRACBITS), getTextureHeight(toptexture));
        }

        if (worldlow > worldbottom) // bottom texture
        {
            // next-hack: texturetranslation are now offsets to index.
            bottomtexture = texturetranslation[getSideBottomTexture(linedef, sidedef)] + getSideBottomTexture(linedef, sidedef);
            rw_bottomtexturemid =
                    linedef->flags & ML_DONTPEGBOTTOM ? worldtop : worldlow;

            rw_bottomtexturemid += FixedMod((sidedef->rowoffset << FRACBITS), getTextureHeight(bottomtexture));
        }
        ...
        {
            maskedtexture = true;
            maskedData.maskedtexturecol = _g->lastopening - rw_x;
            ds_p->maskedtexturecol_sptrb = (int) maskedData.maskedtexturecol - (int32_t) &maskedData;//openingAddrToSsptr(maskedData.maskedtexturecol);
...
// 2021-02-13 next-hack: now textureoffsets are stored in RAM in a separate array. sides are stored in flash
    ...
    if (FIXED16_TO_FIXED32(getRamSector(frontsector)->floorheight16) >= viewz)       // above view plane
        markfloor = false;
    if (FIXED16_TO_FIXED32(getRamSector(frontsector)->ceilingheight16) <= viewz && frontsector->ceilingpic != _g->skyflatnum) // below view plane
        markceiling = false;
    ...
    if (backsector && didsolidcol)
    {
        ...
        {
            ds_p->silhouette |= SIL_BOTTOM;
            ds_p->bsilheight16 = getRamSector(backsector)->floorheight16;
        }
        if (!(ds_p->silhouette & SIL_TOP))
        {
            ds_p->silhouette |= SIL_TOP;
            ds_p->tsilheight16 = getRamSector(backsector)->ceilingheight16;
        }
    }
    ...
    if (((ds_p->silhouette & SIL_TOP) || maskedtexture) && !ds_p->sprtopclip_ssptr)
    {
        ...
        fastOpeningsCopy(_g->lastopening, &maskedData.ceilingclip[start], (rw_stopx - start));
    ...
    if (((ds_p->silhouette & SIL_BOTTOM) || maskedtexture) && !ds_p->sprbottomclip_ssptr)
    {
        ...
        fastOpeningsCopy(_g->lastopening, &maskedData.floorclip[start], (rw_stopx - start));
...
// next-hack: removed flags and validcount usage. Slower but saves memory
static int R_RecalcLineFlags(void)
{
//    linedata_t *linedata = &_g->linedata[linedef->lineno];
    ...
    if (!(linedef->flags & ML_TWOSIDED) 
        || getRamSector(backsector)->ceilingheight16 <= getRamSector(frontsector)->floorheight16 
        || getRamSector(backsector)->floorheight16 >= getRamSector(frontsector)->ceilingheight16 
        || 
        (
            // if door is closed because back is shut:
            getRamSector(backsector)->ceilingheight16 <= getRamSector(backsector)->floorheight16

            // preserve a kind of transparent door/lift special effect:
            && (getRamSector(backsector)->ceilingheight16 >= getRamSector(frontsector)->ceilingheight16 
            || getSideTopTexture(linedef, side))

            && (getRamSector(backsector)->floorheight16 <= getRamSector(frontsector)->floorheight16 || getSideBottomTexture(linedef, side)
        )
    ...
    else
    {
        ...
        if (getRamSector(backsector)->ceilingheight16 != getRamSector(frontsector)->ceilingheight16 
        || getRamSector(backsector)->floorheight16 != getRamSector(frontsector)->floorheight16 
            || getSideMidTexture(linedef, side) 
            || backsector->ceilingpic != frontsector->ceilingpic 
            || getRamSector(backsector)->floorpic != getRamSector(frontsector)->floorpic 
            || getRamSector(backsector)->lightlevel != getRamSector(frontsector)->lightlevel)
        {
//            linedata->r_flags = (linedata->r_flags & RF_MAPPED);
            return 0; // 2020-03-14 next-hack was ML_MAPPED, changed to RF_MAPPED
        }
        else
        {
...
static void R_ClipWallSegment(int first, int last, boolean solid)
{
    ...
    while (first < last)
    {
        ...
        else
        {
            ...
            R_StoreWallRange(first, to - 1, solid); // added solid information
...
static void R_AddLine(uint16_t lineIndex) //(const seg_t *line)
{
    const seg_t *line = &_g->segs[lineIndex];
    int x1;
    int x2;
    angle_t angle1;
    angle_t angle2;
    ...
    int flags = R_RecalcLineFlags();
    if (!(flags & RF_IGNORE))
    {
        R_ClipWallSegment(x1, x2, flags & RF_CLOSED);
    }
...
static void R_Subsector(int num)
{
    ...
    frontsector = &_g->sectors[sub->sector_num];
    count = sub->numlines;
    curlineIndex = sub->firstline;
    //line = &_g->segs[sub->firstline];
    if (FIXED16_TO_FIXED32(getRamSector(frontsector)->floorheight16) < viewz)
    {
        floorplane = R_FindPlane((getRamSector(frontsector)->floorheight16), getRamSector(frontsector)->floorpic, getRamSector(frontsector)->lightlevel // killough 3/16/98
        );
    }
    else
    {
    ...
    if (FIXED16_TO_FIXED32(getRamSector(frontsector)->ceilingheight16) > viewz || (frontsector->ceilingpic == _g->skyflatnum))
    {
        ceilingplane = R_FindPlane((getRamSector(frontsector)->ceilingheight16), // killough 3/8/98
        frontsector->ceilingpic, getRamSector(frontsector)->lightlevel);
    }
    else
    {
        ceilingplane = NULL;
    ...
    R_AddSprites(sub, getRamSector(frontsector)->lightlevel);
    while (count--)
    {

        R_AddLine(curlineIndex);
        curlineIndex++;
        curline = NULL; /* cph 2001/11/18 - must clear curline now we're done with it, so R_ColourMap doesn't try using it for other things */
    }
}
...
static void R_RenderBSPNode(int bspnum)
{
    ...
    //
    byte solidcol_array[SCREENWIDTH]; 
    //
    solidcol = solidcol_array;
    R_ClearClipSegs(); // next-hack: moved here because solidcol is in stack.
...
static void R_DrawPlanes(void)
{
    ...
    {
        visplane_t *pl = getLongPtr(_g->visplanes_sptr[i]);

        while (pl)
        {
            if (isPlaneModified(pl))
                R_DoDrawPlane(pl);

            pl = getLongPtr(pl->next_sptr);
        }
    }
}
...
static void R_ClearPlanes(void)
{
    ...
    for (i = 0; i < MAXVISPLANES; i++)    // new code -- killough
        for (*_g->freehead_psptr = (uint32_t) _g->visplanes_sptr[i], _g->visplanes_sptr[i] = 0; *_g->freehead_psptr;)
            _g->freehead_psptr = &(((visplane_t*)getLongPtr(*_g->freehead_psptr)))->next_sptr;
    _g->lastopening = maskedData.openings;
...
// R_RenderView
//
#define UPDATE_NET_DURING_RENDERING 1
void R_RenderPlayerView(player_t *player)
{
   ...
   uint32_t tn;
    ...
    // Clear buffers.
    //R_ClearClipSegs(); <--- next-hack REMOVED because solidcol is in stack, and used directly in R_RenderBSPNode
    R_ClearDrawSegs();
    R_ClearPlanes();
    R_ClearSprites();
#if UPDATE_NET_DURING_RENDERING
    NetUpdate();
#endif
...
void V_DrawPatchNoScale(int x, int y, const patch_t *patch)
{
    ...
    else
    {
        ...
        //
        patch_t pdata;
        extMemGetDataFromCurrentAddress(&pdata, 4 * sizeof(pdata.width));
        width = pdata.width;
        // remove offset
        x -= pdata.leftoffset;    // patch->leftoffset;
        y -= pdata.topoffset; // patch->topoffset;
        ...
        {
            extMemGetDataFromCurrentAddress(columnofs, width * sizeof(columnofs[0]));
        }
        else
        {
            extMemGetDataFromCurrentAddress(columnofs, MAX_PATCH_COLOFFS * sizeof(columnofs[0]));
        }
        byte *desttop = (byte*) _g->screens[0].data;
        ...
        for (unsigned int colindex = 0; colindex < width; colindex++, desttop++)
        {
            ...
            if (colindex - coloffset >= MAX_PATCH_COLOFFS)
            {
                ...
                coloffset = colindex;
                extMemSetCurrentAddress((uint32_t) patch + 8 + 4 * coloffset);
                if (width - coloffset <= MAX_PATCH_COLOFFS)
                {
                    extMemGetDataFromCurrentAddress(columnofs, (width - coloffset) * sizeof(columnofs[0]));
                }
                else
                {
                    extMemGetDataFromCurrentAddress(columnofs, MAX_PATCH_COLOFFS * sizeof(columnofs[0]));
            ...
            extMemSetCurrentAddress((uint32_t) patch + (0xFFFFFF & columnofs[colindex - coloffset]));

            // read column
            uint8_t columnData[MAX_COLUMN_DATA];
            extMemGetDataFromCurrentAddress(columnData, columnofs[colindex - coloffset] >> 24);

            const column_t *column = (const column_t*) columnData;
...
static boolean P_CrossSubsector(int num)
{
    ...
    { // check lines
        ...
        if (getBitMask32(_g->lineSectorChecked, linenum))
            continue;
        
        setBitMask32(_g->lineSectorChecked, linenum);
        ...
        if (line->flags & ML_TWOSIDED)
        {
            ...
            if (getRamSector((front = SG_FRONTSECTOR(seg)))->floorheight16 == getRamSector((back = SG_BACKSECTOR(seg)))->floorheight16 && getRamSector(front)->ceilingheight16 == getRamSector(back)->ceilingheight16)
                continue;

            // possible occluder
            // because of ceiling height differences
            opentop =
                    getRamSector(front)->ceilingheight16 < getRamSector(back)->ceilingheight16 ? FIXED16_TO_FIXED32(getRamSector(front)->ceilingheight16) : FIXED16_TO_FIXED32(getRamSector(back)->ceilingheight16);

            // because of floor height differences
            openbottom =
                    getRamSector(front)->floorheight16 > getRamSector(back)->floorheight16 ? FIXED16_TO_FIXED32(getRamSector(front)->floorheight16) : FIXED16_TO_FIXED32(getRamSector(back)->floorheight16);
        ...
        if (getRamSector(front)->floorheight16 != getRamSector(back)->floorheight16)
        {
        ...
        if (getRamSector(front)->ceilingheight16 != getRamSector(back)->ceilingheight16)
        {
...
boolean P_SetMobjState(mobj_t *mobj, statenum_t state)
{
    ...
    do
    {
        ...
        if (st->action)
        {
            if (!(_g->players[_g->consoleplayer].cheats & CF_ENEMY_ROCKETS))
            {
                st->action(mobj);
            }
            else
...
void THINKFUN_ALIGN4 P_MobjThinker(mobj_t *mobj)
{
    ...
    {
        // momentum movement
        if (mobj->momx || mobj->momy || (getMobjFlags(mobj) & MF_SKULLFLY )) // it was momx | momy, but it should have been ||
        {
            P_XYMovement(mobj);
            if (mobj->thinker.function_idx != THINKER_IDX(P_MobjThinker)) // cph - Must've been removed
                return;       // killough - mobj was removed
        }
        if (mobj->zr != FIXED16_TO_FIXED_Z(mobj->floorz16) || mobj->momz16)
        {
            P_ZMovement(mobj);
            if (mobj->thinker.function_idx != THINKER_IDX(P_MobjThinker)) // cph - Must've been removed
                return;       // killough - mobj was removed
    ...
    else
    {
        ...
        if (!(getMobjFlags(mobj) & MF_COUNTKILL ))
            return;
        ...
        if (mobj->movecount < 12 * 35)
            ...
            mobj->movecount = 12 * 35;      // next-hack: avoid overflow
...
void P_RunThinkers(void)
{
    ...
    while(th != th_end)
    {
        ...
        if(th->function_idx)
        {
            if (th->function_idx == THINKER_IDX(P_RemoveStaticThinkerDelayed))
            {
                P_RemoveStaticThinkerDelayed(th, prev_th);
            }
            else if (th->function_idx == THINKER_IDX(P_RemoveThinkerDelayed))
            {
                P_RemoveThinkerDelayed(th, prev_th);
            }
            else
            {
                void (* thinkerFunction)(void*) = getThinkerFunction(th);
                thinkerFunction(th);
                prev_th = th;
            }
        }
