 ...
 *
 *      next-hack: modified code to work with short pointers, optimized a lot
 *      and added kill messages.
 *
 *-----------------------------------------------------------------------------*/
...
boolean P_GivePower(player_t *player, int power)
{
    ...
    switch (power)
    {
        case pw_invisibility:
            setMobjFlagsBits(player->mo, MF_SHADOW, SET_FLAGS);
            break;
        case pw_allmap:
...
void P_TouchSpecialThing(mobj_t *special, mobj_t *toucher)
{
    ...
    int i;
    int sound;
    fixed_t delta = FIXED_Z_TO_FIXED32(special->zr - toucher->zr);
    ...
    switch (getMobjSprite(special))
    {
        ...
        case SPR_PVIS:
            ...
            // ammo
        case SPR_CLIP:
            if (getMobjFlags(special) & MF_DROPPED)
            {
        ...
        case SPR_MGUN:
            if (!P_GiveWeapon(player, wp_chaingun, (getMobjFlags(special) & MF_DROPPED ) != 0))
                return;
        ...
        case SPR_SHOT:
            if (!P_GiveWeapon(player, wp_shotgun, (getMobjFlags(special) & MF_DROPPED ) != 0))
                return;
        ...
        case SPR_SGN2:
            if (!P_GiveWeapon(player, wp_supershotgun, (getMobjFlags(special) & MF_DROPPED ) != 0))
                return;
    ...
    if (getMobjFlags(special) & MF_COUNTITEM)
        player->itemcount++;
    ...
    if (player == &_g->players[_g->displayplayer])
...
// next-hack: added message to notify all when a player kill occurs.
static char killMessage[48];
// todo: shall we player names instead ?
static const char * playerColors[] = {"green", "indigo", "brown", "red"};
static void P_KillMobj(mobj_t *source, mobj_t *target)
{
    mobjtype_t item;
    mobj_t *mo;

    setMobjFlagsBits(target, (MF_SHOOTABLE | MF_FLOAT | MF_SKULLFLY ), CLEAR_FLAGS);

    demodbgprintf("KillMobj target type %d\r\n",target->type);

    if (target->type != MT_SKULL)
        setMobjFlagsBits(target, MF_NOGRAVITY, CLEAR_FLAGS);
    setMobjFlagsBits(target, MF_CORPSE | MF_DROPOFF, SET_FLAGS);
    target->height_s >>= 2;

    if (!((getMobjFlags(target) ^ MF_COUNTKILL ) & (MF_FRIEND | MF_COUNTKILL )))
        _g->totallive--;

    if (source && getMobjPlayer(source))
    {
        // count for intermission
        if (getMobjFlags(target) & MF_COUNTKILL)
            getMobjPlayer(source)->killcount++;

        if (getMobjPlayer(target))
        {
            getMobjPlayer(source)->frags[getMobjPlayer(target) - _g->players]++;
            if (target->player_n == _g->consoleplayer)
            { // you were killed
              // TODO: put player name!
              if (target->player_n == source->player_n)
              {
                snprintf(killMessage, sizeof(killMessage), "You killed yourself!");
              }
              else
              {
                snprintf(killMessage, sizeof(killMessage), "You were killed by player %s!", playerColors[source->player_n]);
              }
              _g->players[_g->consoleplayer].message = killMessage;
            }
            else if (source->player_n != _g->consoleplayer)
            { // one player killed another
              if (source->player_n != target->player_n)
              {
                snprintf(killMessage, sizeof(killMessage), "Player %s was killed by player %s!", playerColors[target->player_n], playerColors[source->player_n]);
              }
              else
              {
                snprintf(killMessage, sizeof(killMessage), "Player %s was tired of living.", playerColors[source->player_n]);
              }
              _g->players[_g->consoleplayer].message = killMessage;
            }
            else
            {  // you killed
              snprintf(killMessage, sizeof(killMessage), "You killed player %s!", playerColors[target->player_n]);
              _g->players[_g->consoleplayer].message = killMessage;
            }
        }
    }
    else if (getMobjFlags(target) & MF_COUNTKILL)
    { /* Add to kills tally */
    ...
    if (getMobjPlayer(target))
    {
      ...
      if (!source)
          getMobjPlayer(target)->frags[getMobjPlayer(target) - _g->players]++;

        setMobjFlagsBits(target, MF_SOLID, CLEAR_FLAGS);
        getMobjPlayer(target)->playerstate = PST_DEAD;
        P_DropWeapon(getMobjPlayer(target));

        if (getMobjPlayer(target) == &_g->players[_g->consoleplayer] && (_g->automapmode & am_active))
    ...
    mo = P_SpawnMobj(target->x, target->y, FIXED16_TO_FIXED32((unsigned int) ONFLOORZ16), item | MOBJ_TYPE_DROPPED);
    setMobjFlagsBits(mo, MF_DROPPED, SET_FLAGS);    // special versions of items
}
...
void P_DamageMobj(mobj_t *target, mobj_t *inflictor, mobj_t *source, int damage)
{
    ...
    if (!(getMobjFlags(target) & (MF_SHOOTABLE )))
        return; // shouldn't happen...

    if (target->health <= 0)
        return;

    if (getMobjFlags(target) & MF_SKULLFLY)
        target->momx = target->momy = target->momz16 = 0;
    ...
    if (inflictor && !(getMobjFlags(target) & MF_NOCLIP ) && (!source || !getMobjPlayer(source) || getMobjPlayer(source)->readyweapon != wp_chainsaw))
    {
        ...
        if (damage < 40 && damage > target->health && FIXED_Z_TO_FIXED32(target->zr - inflictor->zr) > 64 * FRACUNIT && (P_Random() & 1))
        {
            ang += ANG180;
            thrust *= 4;
        }
    ...
    if (player)
    {
        ...
        if (_g->ramsectors[getMobjSubsector(target)->sector_num].special == 11 && damage >= target->health)
    ...
    if (target->health <= 0)
    {
        ...
        printf("Killed Mobj at gametic %d, basetic %d\r\n", _g->gametic, _g->basetic);
        return;
    }
    if (!demo_compatibility)
    {
        ...
        if (P_Random() < getMobjInfo(target)->painchance && !(getMobjFlags(target) & MF_SKULLFLY ))
        ...
        if (source && source != target && source->type != MT_VILE && (!target->threshold || target->type == MT_VILE))
        {
            ...
            if (0 == target->lastenemy_sptr || getLastEnemy(target)->health <= 0 || (!((getMobjFlags(target) ^ getMobjFlags(getLastEnemy(target))) & MF_FRIEND ) && getTarget(target) != source)) // remember last enemy - killough
            {
        ...
        if (justhit && (getTarget(target) == source || !getTarget(target) || !(getMobjFlags(target) & getMobjFlags(getTarget(target)) & MF_FRIEND )))
            setMobjFlagsBits(target, MF_JUSTHIT, SET_FLAGS);    // fight back!
    }
    else
    {
        if (P_Random() < getMobjInfo(target)->painchance && !(getMobjFlags(target) & MF_SKULLFLY ))
