...
static void R_InitSpriteDefs(const char *const*namelist)
{
    ...
    {                                      // prepend so that later ones win
        char sn[8];
        extMemSetCurrentAddress((uint32_t) W_GetNameForNum(i + _g->firstspritelump));
        extMemGetDataFromCurrentAddress(sn, sizeof(sn));
    ...
    for (i = 0; i < numsprites; i++)
    {
        ...
        if (j >= 0)
        {
            ...
            {
                char sn[8];
                extMemSetCurrentAddress((uint32_t) W_GetNameForNum(j + _g->firstspritelump));
                extMemGetDataFromCurrentAddress(sn, sizeof(sn));
