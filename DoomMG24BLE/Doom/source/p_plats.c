 ...
 *
 *  next-hack: modified code to work with short pointers, plus other optimizations.
 *-----------------------------------------------------------------------------*/
...
void THINKFUN_ALIGN4 T_PlatRaise(plat_t *plat)
{
    result_e res;
    ...
    switch (plat->status)
    {
        ...
        case waiting: // plat is waiting
            ...
            {
                if (getRamSector(plat->sector)->floorheight16 == FIXED32_TO_FIXED16(plat->low))
...
int EV_DoPlat(const line_t *line, plattype_e type, int amount)
{
    ...
    while ((secnum = P_FindSectorFromLineTag(line, secnum)) >= 0)
    {
        ...
        plat->type = type;
        plat->sector = sec;
        getRamSector(plat->sector)->floordata_sptr = getShortPtr(plat); //jff 2/23/98 multiple thinkers
        ...
        switch (type)
        {
            case raiseToNearestAndChange:
                ...
                getRamSector(sec)->floorpic = _g->ramsectors[_g->sides[line->sidenum[0]].sector_num].floorpic;
                plat->high = P_FindNextHighestFloor(sec, FIXED16_TO_FIXED32(getRamSector(sec)->floorheight16));
                plat->wait = 0;
                plat->status = up;
                getRamSector(sec)->special = 0;
                //jff 3/14/98 clear old field as well
                getRamSector(sec)->wasSecret = 0;
            ...
            case raiseAndChange:
                plat->speed = PLATSPEED / 2;
                getRamSector(sec)->floorpic = _g->ramsectors[_g->sides[line->sidenum[0]].sector_num].floorpic;
                plat->high = FIXED16_TO_FIXED32(getRamSector(sec)->floorheight16) + amount * FRACUNIT;
                plat->wait = 0;
                plat->status = up;
            ...
            case toggleUpDn: //jff 3/14/98 add new type to support instant toggle
                ...
                plat->low = FIXED16_TO_FIXED32(getRamSector(sec)->ceilingheight16);
                plat->high = FIXED16_TO_FIXED32(getRamSector(sec)->floorheight16);
                plat->status = down;
                break;
...
int EV_StopPlat(const line_t *line)
{
    ...
    for (pl = activeplats; pl; pl = pl->next)  // search the active plats
    {
        ...
        if (plat->status != in_stasis && plat->tag == line->tag)
        {
            ...
            plat->thinker.function_idx = 0; // NULL
        }
    }
    return 1;
}
...
void P_RemoveActivePlat(plat_t *plat)
{
    ...
    getRamSector(plat->sector)->floordata_sptr = 0; //jff 2/23/98 multiple thinkers
