...
static const uint8_t  their_colors[] = { GREENS, GRAYS, BROWNS, REDS };
...
//        else if (ch == key_map_follow && _g->gamekeydown[key_use])
        else if (ch == key_map_follow && (_g->gamekeydown & (1 << key_use)))
        {
...
static void AM_drawWalls(void)
{
    ...
    for (i = 0; i < _g->numlines; i++)
    {
        ...
        {
            AM_rotate(&l.a.x, &l.a.y, ANG90 - ANGLE16_TO_ANGLE32(_g->players[_g->consoleplayer].mo->angle16), _g->players[_g->consoleplayer].mo->x, _g->players[_g->consoleplayer].mo->y);
            AM_rotate(&l.b.x, &l.b.y, ANG90 - ANGLE16_TO_ANGLE32(_g->players[_g->consoleplayer].mo->angle16), _g->players[_g->consoleplayer].mo->x, _g->players[_g->consoleplayer].mo->y);
        }
...
#else
        if (getBitMask32(_g->lineIsMapped, i))
#endif
        {
            ...
            else /* now for 2S lines */
            {
                ...
                else if (_g->lines[i].flags & ML_SECRET) // secret door
                {
                    AM_drawMline(&l, mapcolor_wall); // wall color
                         ...
                         ((getRamSector(backsector)->floorheight16 == getRamSector(backsector)->ceilingheight16) ||
                          (getRamSector(frontsector)->floorheight16 == getRamSector(frontsector)->ceilingheight16)))
                {
                ...
                else if (getRamSector(backsector)->floorheight16 != getRamSector(frontsector)->floorheight16)
                {
                    AM_drawMline(&l, mapcolor_fchg); // floor level change
                }
                else if (getRamSector(backsector)->ceilingheight16 != getRamSector(frontsector)->ceilingheight16)
                {
        ...
        else if (_g->players[_g->consoleplayer].powers[pw_allmap]) // computermap visible lines
        {
            ...
            {
                if (mapcolor_flat || !backsector || getRamSector(backsector)->floorheight16 != getRamSector(frontsector)->floorheight16 ||
...
static void AM_drawPlayers(void)
{
  ...
  if (!_g->netgame)
  {
    AM_drawLineCharacter(player_arrow, NUMPLYRLINES, 0, ANGLE16_TO_ANGLE32(_g->players[_g->consoleplayer].mo->angle16), mapcolor_sngl, // jff color
            _g->players[_g->consoleplayer].mo->x >> FRACTOMAPBITS,                                                   // e6y
            _g->players[_g->consoleplayer].mo->y >> FRACTOMAPBITS);                                                  // e6y    return;
    return;
  }

  for (i = 0; i < MAXPLAYERS; i++)
  {
    player_t* p = &_g->players[i];
    // don't draw other players in deathmatch mode
    if ( (_g->deathmatch && !_g->demoplayback) && p != &_g->players[_g->consoleplayer])
      continue;

    if (_g->playeringame[i])
    {
                                                 // e6y
      AM_drawLineCharacter (player_arrow, NUMPLYRLINES, 0, ANGLE16_TO_ANGLE32(p->mo->angle16),
          p->powers[pw_invisibility] ? 246 /* *close* to black */
          : their_colors[i], //jff 1/6/98 use default color
          p->mo->x >> FRACTOMAPBITS,                                                   // e6y
          p->mo->y >> FRACTOMAPBITS);                                                  // e6y    return;
   }
  }
}
