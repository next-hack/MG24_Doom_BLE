 ...
 * 2023: now lump addresses are directly saved (yes, we could avoid this and
 * save some 16 bits), together with audio data length and increment (22050-Hz
 * samples have increment 2).
 *
 *-----------------------------------------------------------------------------*/
...
static const int numChannels = MAX_CHANNELS;
//
typedef struct
{
    uint16_t format;
...
void S_Init(int sfxVolume, int musicVolume)
{
    ...
    soundLumpData_t *soundLumps = Z_Malloc(NUMSFX * sizeof(soundLumpData_t), PU_STATIC, NULL);
    for (int i = 0; i < NUMSFX; i++)
    {
        int lumpNum = W_CheckNumForName(S_sfx[i].name);
        if (lumpNum >= 0)
        {
          soundLumps[i].lumpAddress = W_CacheLumpNum(lumpNum);
        }
        else
        {
          soundLumps[i].lumpAddress = 0;
        }
        if (isOnExternalFlash(soundLumps[i].lumpAddress))
        {
          extMemSetCurrentAddress((uint32_t)soundLumps[i].lumpAddress);
          doomSoundHeader_t dsh;
          extMemGetDataFromCurrentAddress(&dsh, sizeof (doomSoundHeader_t));
          if (dsh.sampleRate == 22050)
          {
            soundLumps[i].increment = 2;
          }
          else
          {
            soundLumps[i].increment = 1;
          }
          soundLumps[i].length = W_LumpLength(lumpNum) - DMX_DATA_SOUND_OFFSET;
        }
        //

    }
    p_wad_immutable_flash_data->soundLumps = writeBufferToFlashRegion(soundLumps, NUMSFX * sizeof(soundLumpData_t), FLASH_IMMUTABLE_REGION, true);
    Z_Free(soundLumps);
#if ENABLE_MUSIC
    OPL2_Reset(0);
    OPL_InitRegisters();
    initMusic();
    uint32_t *musicLumpAddr = Z_Calloc(NUMMUSIC , sizeof(uint32_t), PU_STATIC, NULL);
    for (int i = 0; i < NUMMUSIC; i++)  // note: must start from 1. First music is empty
    {
        int num = W_CheckNumForName(S_music[i]);
        uint32_t addr = 0;
        uint32_t offset = 0;
        if (num != -1)
        {
          addr = (uint32_t) W_CacheLumpNum(num);
          mus_t mus;
          extMemSetCurrentAddress(addr);
          extMemGetDataFromCurrentAddress(&mus, sizeof(mus));
          offset = mus.offSong;
        }
        musicLumpAddr[i] = addr + offset;
        printf("Mus %i address %x offs %x tot %x\r\n", i, addr, offset, musicLumpAddr[i]);
    }
    p_wad_immutable_flash_data->musicAddrs = writeBufferToFlashRegion(musicLumpAddr, NUMMUSIC * sizeof(uint32_t), FLASH_IMMUTABLE_REGION, true);
    Z_Free(soundLumps);

    p_wad_immutable_flash_data->instruments = (genmidi_instr_t *)((uint8_t *) writeLumpToFlashRegion(W_GetNumForName("GENMIDI"), FLASH_IMMUTABLE_REGION, true) + GENMIDI_HEADER_SIZE);
...
//        priority = NORM_PRIORITY;
    ...
    if (!origin || origin == (sound_mobj_t*)_g->players[_g->displayplayer].mo)
    {
        volume *= 8;
    }
    else
...
void S_StopChannel(int cnum)
{
    ...
    soundChannels[cnum].volumeLeft = 0;
}
...
int S_AdjustSoundParams(mobj_t *listener, sound_mobj_t *source, int *vol, int *sep)
{
    ...
    if (angle <= ANGLE16_TO_ANGLE32(listener->angle16))
        angle += 0xffffffff;

    angle -= ANGLE16_TO_ANGLE32(listener->angle16);
