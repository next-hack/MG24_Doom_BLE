 ...
 *
 *  next-hack: modified code to work with short pointers, ram sectors, and
 *  optimized a lot memory.
 *
 *-----------------------------------------------------------------------------*/
#include "doomstat.h"
#include "r_main.h"
    ...
    switch (floorOrCeiling)
    {
        case 0:
            ...
            switch (direction)
            {
                case -1:
                    // Moving a floor down
                    if (FIXED16_TO_FIXED32(getRamSector(sector)->floorheight16) - speed < dest)
                    {
                        lastpos = FIXED16_TO_FIXED32(getRamSector(sector)->floorheight16);
                        getRamSector(sector)->floorheight16 = FIXED32_TO_FIXED16(dest);
                        flag = P_CheckSector(sector, crush); //jff 3/19/98 use faster chk
                        if (flag == true)
                        {
                            getRamSector(sector)->floorheight16 = FIXED32_TO_FIXED16(lastpos);
                    ...
                    {
                        lastpos = FIXED16_TO_FIXED32(getRamSector(sector)->floorheight16);
                        getRamSector(sector)->floorheight16 -= FIXED32_TO_FIXED16(speed);
                        flag = P_CheckSector(sector, crush); //jff 3/19/98 use faster chk
                ...
                case 1:
                    ...
                    destheight =
                            (dest < FIXED16_TO_FIXED32(getRamSector(sector)->ceilingheight16)) ? dest : FIXED16_TO_FIXED32(getRamSector(sector)->ceilingheight16);
                    if (FIXED16_TO_FIXED32(getRamSector(sector)->floorheight16) + speed > destheight)
                    {
                        lastpos = FIXED16_TO_FIXED32(getRamSector(sector)->floorheight16);
                        getRamSector(sector)->floorheight16 = FIXED32_TO_FIXED16(destheight);
                        flag = P_CheckSector(sector, crush); //jff 3/19/98 use faster chk
                        if (flag == true)
                        {
                            getRamSector(sector)->floorheight16 = FIXED32_TO_FIXED16(lastpos);
        ...
        case 1:
            ...
            switch (direction)
            {
                case -1:
                    ...
                    // jff 2/22/98 weaken check to demo_compatibility
                    destheight =
                            (dest > FIXED16_TO_FIXED32(getRamSector(sector)->floorheight16)) ? dest : FIXED16_TO_FIXED32(getRamSector(sector)->floorheight16);
                    if (FIXED16_TO_FIXED32(getRamSector(sector)->ceilingheight16) - speed < destheight)
                    {
                        lastpos = FIXED16_TO_FIXED32(getRamSector(sector)->ceilingheight16);
                        getRamSector(sector)->ceilingheight16 = FIXED32_TO_FIXED16(destheight);
                        flag = P_CheckSector(sector, crush); //jff 3/19/98 use faster chk

                        if (flag == true)
                        {
                            getRamSector(sector)->ceilingheight16 = FIXED32_TO_FIXED16(lastpos);
                            P_CheckSector(sector, crush); //jff 3/19/98 use faster chk
                        }
                        return pastdest;
                    }
                    else
                    {
                        // crushing is possible
                        lastpos = FIXED16_TO_FIXED32(getRamSector(sector)->ceilingheight16);
                        getRamSector(sector)->ceilingheight16 -= FIXED32_TO_FIXED16(speed);
                        flag = P_CheckSector(sector, crush); //jff 3/19/98 use faster chk
                ...
                case 1:
                    // moving a ceiling up
                    if (FIXED16_TO_FIXED32(getRamSector(sector)->ceilingheight16) + speed > dest)
                    {
                        lastpos = FIXED16_TO_FIXED32(getRamSector(sector)->ceilingheight16);
                        getRamSector(sector)->ceilingheight16 = FIXED32_TO_FIXED16(dest);
                        flag = P_CheckSector(sector, crush); //jff 3/19/98 use faster chk
                        if (flag == true)
                        {
                            getRamSector(sector)->ceilingheight16 = FIXED32_TO_FIXED16(lastpos);
...
void THINKFUN_ALIGN4 T_MoveFloor(floormove_t *floor)
{
    result_e res;
    ...
    if (res == pastdest)    // if destination height is reached
    {
        if (floor->direction == 1)       // going up
        {
            ...
            {
                case donutRaise:
                    getRamSector(getFloorMoveSector(floor))->special = floor->newspecial;
                    getFloorMoveRamSector(floor)->floorpic = floor->texture;
                    break;
                case genFloorChgT:
                case genFloorChg0:
                    getFloorMoveRamSector(floor)->special = floor->newspecial;
                    //jff add to fix bug in special transfers from changes
                    getFloorMoveRamSector(floor)->wasSecret = floor->oldspecial;
                    // fall through
                case genFloorChg:
                    getFloorMoveRamSector(floor)->floorpic = floor->texture;
                    break;
        ...
        getFloorMoveRamSector(floor)->floordata_sptr = 0; //jff 2/22/98
...
//
void THINKFUN_ALIGN4 T_MoveElevator(elevator_t *elevator)
{
    result_e res;
    ...
    {
        getRamSector(elevator->sector)->floordata_sptr = 0;     //jff 2/22/98
        getRamSector(elevator->sector)->ceilingdata_sptr = 0;   //jff 2/22/98
...
int EV_DoFloor(const line_t *line, floor_e floortype)
{
    ...
    while ((secnum = P_FindSectorFromLineTag(line, secnum)) >= 0)
    {
        ...
        getRamSector(sec)->floordata_sptr = getShortPtr(floor); //jff 2/22/98
        floor->thinker.function_idx = THINKER_IDX(T_MoveFloor);
        ...
        switch (floortype)
        {
            case lowerFloor:
                floor->direction = -1;
                floor->sector_number = sec->sectorNumber;
            ...
            case lowerFloor24:
                ...
                floor->sector_number = sec->sectorNumber;
                floor->speed = FLOORSPEED;
                floor->floordestheight = FIXED16_TO_FIXED32(getFloorMoveRamSector(floor)->floorheight16) + 24 * FRACUNIT;
                break;
            ...
            case lowerFloor32Turbo:
                ...
                floor->floordestheight = FIXED16_TO_FIXED32(getFloorMoveRamSector(floor)->floorheight16) + 32 * FRACUNIT;
                break;
            ...
            case lowerFloorToNearest:
                ...
                floor->sector_number = sec->sectorNumber;
                floor->speed = FLOORSPEED;
                floor->floordestheight = P_FindNextLowestFloor(sec, FIXED16_TO_FIXED32(getFloorMoveRamSector(floor)->floorheight16));
                break;
            ...
            case turboLower:
                ...
                floor->speed = FLOORSPEED * 4;
                floor->floordestheight = P_FindHighestFloorSurrounding(sec);
                if (floor->floordestheight != FIXED16_TO_FIXED32(getRamSector(sec)->floorheight16))
            ...
            case raiseFloor:
                ...
                if (floor->floordestheight > FIXED16_TO_FIXED32(getRamSector(sec)->ceilingheight16))
                    floor->floordestheight = FIXED16_TO_FIXED32(getRamSector(sec)->ceilingheight16);
            ...
            case raiseFloorTurbo:
                ...
                floor->sector_number = sec->sectorNumber;
                floor->speed = FLOORSPEED * 4;
                floor->floordestheight = P_FindNextHighestFloor(sec, FIXED16_TO_FIXED32(getRamSector(sec)->floorheight16));
                break;
            ...
            case raiseFloor512:
                ...
                floor->sector_number = sec->sectorNumber;
                floor->speed = FLOORSPEED;
                floor->floordestheight = FIXED16_TO_FIXED32(getFloorMoveRamSector(floor)->floorheight16) + 512 * FRACUNIT;
                break;
            ...
            case raiseFloor24AndChange:
                ...
                floor->speed = FLOORSPEED;
                floor->floordestheight = FIXED16_TO_FIXED32(getFloorMoveRamSector(floor)->floorheight16) + 24 * FRACUNIT;
                getRamSector(sec)->floorpic = getRamSector(LN_FRONTSECTOR(line))->floorpic;
                getRamSector(sec)->special = getRamSector(LN_FRONTSECTOR(line))->special;
                //jff 3/14/98 transfer both old and new special
                getRamSector(sec)->wasSecret = getRamSector(LN_FRONTSECTOR(line))->wasSecret;
                break;
            ...
            case raiseToTexture:
            {
                ...
                for (i = 0; i < sec->linecount; i++)
                {
                    if (twoSided(secnum, i))
                    {
                        ...
                        if (side->bottomtexture > 0)
                            if (getTextureHeight(side->bottomtexture) < minsize)
                                minsize = getTextureHeight(side->bottomtexture);
                ...
                {
                    floor->floordestheight = (FIXED16_TO_FIXED32(getFloorMoveRamSector(floor)->floorheight16) >> FRACBITS) + (minsize >> FRACBITS);
            ...
            case lowerAndChange:
                ...
                floor->speed = FLOORSPEED;
                floor->floordestheight = P_FindLowestFloorSurrounding(sec);
                floor->texture = getRamSector(sec)->floorpic;

                // jff 1/24/98 make sure floor->newspecial gets initialized
                // in case no surrounding sector is at floordestheight
                // --> should not affect compatibility <--
                floor->newspecial = getRamSector(sec)->special;
                //jff 3/14/98 transfer both old and new special
                floor->oldspecial = getRamSector(sec)->wasSecret;

                //jff 5/23/98 use model subroutine to unify fixes and handling
//                sec = P_FindModelFloorSector(floor->floordestheight, sec - _g->sectors);
                sec = P_FindModelFloorSector(floor->floordestheight, sec->sectorNumber);
                if (sec)
                {
                    floor->texture = getRamSector(sec)->floorpic;
                    floor->newspecial = getRamSector(sec)->special;
    ...
    while ((secnum = P_FindSectorFromLineTag(line, secnum)) >= 0)
    {
        ...
        switch (changetype)
        {
            case trigChangeOnly:
                ...
                break;
            case numChangeOnly:
                secm = P_FindModelFloorSector(FIXED16_TO_FIXED32(getRamSector(sec)->floorheight16), secnum);
                if (secm) // if no model, no change
                {
                    getRamSector(sec)->floorpic = getRamSector(secm)->floorpic;
                    getRamSector(sec)->special = getRamSector(secm)->special;
                    getRamSector(sec)->wasSecret = getRamSector(secm)->wasSecret;
                }
                break;
            default:
                break;
...
int EV_BuildStairs(const line_t *line, stair_e type)
{
    ...
    while ((ssec = P_FindSectorFromLineTagWithLowerBound(line, ssec, minssec)) >= 0)
    {
        ...
        { //jff 2/22/98
            ...
            memset(floor, 0, sizeof(*floor));
            P_AddThinker(&floor->thinker);
            getRamSector(sec)->floordata_sptr = getShortPtr(floor);
            floor->thinker.function_idx = THINKER_IDX(T_MoveFloor);
            floor->direction = 1;
            floor->sector_number = sec->sectorNumber;
            ...
            floor->speed = speed;
            height = FIXED16_TO_FIXED32(getRamSector(sec)->floorheight16) + stairsize;
            floor->floordestheight = height;

            texture = getRamSector(sec)->floorpic;
...
//        sector_t* tsec = LN_FRONTSECTOR((sec->lines[i]));
                    ...
                    if (getRamSector(tsec)->floorpic != texture)
                        continue;
...
//    s2 = getNextSector(s1->lines[0],s1);  // s2 is pool's sector
        ...
        for (i = 0; i < s2->linecount; i++)
        {
            ...
            memset(floor, 0, sizeof(*floor));
            P_AddThinker(&floor->thinker);
            getRamSector(s2)->floordata_sptr = getShortPtr(floor); //jff 2/22/98
            floor->thinker.function_idx = THINKER_IDX(T_MoveFloor);
            floor->type = donutRaise;
            floor->crush = false;
            floor->direction = 1;
            floor->sector_number = s2->sectorNumber;
            floor->speed = FLOORSPEED / 2;
            floor->texture = getRamSector(s3)->floorpic;
            floor->newspecial = 0;
            floor->floordestheight = FIXED16_TO_FIXED32(getRamSector(s3)->floorheight16);

            //  Spawn lowering donut-hole pillar
            floor = Z_Malloc(sizeof(*floor), PU_LEVSPEC, 0);
            memset(floor, 0, sizeof(*floor));
            P_AddThinker(&floor->thinker);
            getRamSector(s1)->floordata_sptr = getShortPtr(floor); //jff 2/22/98
            floor->thinker.function_idx = THINKER_IDX(T_MoveFloor);
            floor->type = lowerFloor;
            floor->crush = false;
            floor->direction = -1;
            floor->sector_number = s1->sectorNumber;
            floor->speed = FLOORSPEED / 2;
            floor->floordestheight = FIXED16_TO_FIXED32(getRamSector(s3)->floorheight16);
            break;
        }
...
int EV_DoElevator(const line_t *line, elevator_e elevtype)
{
    ...
    while ((secnum = P_FindSectorFromLineTag(line, secnum)) >= 0)
    {
        ...
        if (getRamSector(sec)->floordata_sptr || getRamSector(sec)->ceilingdata_sptr) //jff 2/22/98
            continue;
        ...
        getRamSector(sec)->floordata_sptr = getShortPtr(elevator); //jff 2/22/98
        getRamSector(sec)->ceilingdata_sptr = getShortPtr(elevator); //jff 2/22/98
        elevator->thinker.function_idx = THINKER_IDX(T_MoveElevator);
        ...
        switch (elevtype)
        {
            ...
            case elevateDown:
                ...
                elevator->floordestheight = P_FindNextLowestFloor(sec, FIXED16_TO_FIXED32(getRamSector(sec)->floorheight16));
                elevator->ceilingdestheight = elevator->floordestheight + FIXED16_TO_FIXED32(getRamSector(sec)->ceilingheight16 - getRamSector(sec)->floorheight16);
                break;
            ...
            case elevateUp:
                ...
                elevator->sector = sec;
                elevator->speed = ELEVATORSPEED;
                elevator->floordestheight = P_FindNextHighestFloor(sec, FIXED16_TO_FIXED32(getRamSector(sec)->floorheight16));
                elevator->ceilingdestheight = elevator->floordestheight + FIXED16_TO_FIXED32(getRamSector(sec)->ceilingheight16 - getRamSector(sec)->floorheight16);
                break;
            ...
            case elevateCurrent:
                elevator->sector = sec;
                elevator->speed = ELEVATORSPEED;
                elevator->floordestheight = FIXED16_TO_FIXED32(getRamSector(LN_FRONTSECTOR(line))->floorheight16);
                elevator->ceilingdestheight = elevator->floordestheight + FIXED16_TO_FIXED32(getRamSector(sec)->ceilingheight16 - getRamSector(sec)->floorheight16);
                elevator->direction =
                        elevator->floordestheight > FIXED16_TO_FIXED32(getRamSector(sec)->floorheight16) ? 1 : -1;
                break;
