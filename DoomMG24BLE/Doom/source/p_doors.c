...
void THINKFUN_ALIGN4 T_VerticalDoor(vldoor_t *door)
{
    result_e res;
    ...
    switch (door->direction)
    {
        ...
        case -1:
            // Door is moving down
            res = T_MovePlane(door->sector, door->speed, FIXED16_TO_FIXED32(getRamSector(door->sector)->floorheight16),
            ...
            if (door->lighttag && door->topheight - FIXED16_TO_FIXED32(getRamSector(door->sector)->floorheight16))
                EV_LightTurnOnPartway(door->line, FixedDiv(FIXED16_TO_FIXED32(getRamSector(door->sector)->ceilingheight16 - getRamSector(door->sector)->floorheight16), door->topheight - FIXED16_TO_FIXED32(getRamSector(door->sector)->floorheight16)));
            ...
            if (res == pastdest)
            {
                switch (door->type)
                {
                    ...
                    case genBlazeRaise:
                    case genBlazeClose:
                        getRamSector(door->sector)->ceilingdata_sptr = 0;  //jff 2/22/98
...
int EV_VerticalDoor(const line_t *line, mobj_t *thing)
{
    ...
    if (door && ((LN_SPECIAL(line) == 1) || (LN_SPECIAL(line) == 117) || (LN_SPECIAL(line) == 26) || (LN_SPECIAL(line) == 27) || (LN_SPECIAL(line) == 28)))
    {
         ...
         */
        if (door->thinker.function_idx == THINKER_IDX(T_VerticalDoor))
        {
             ...
             */
            if (door->thinker.function_idx == THINKER_IDX(T_VerticalDoor) && door->direction == -1)
            {
                outval = 1; /* go back up */
            }
            else if (player)
            ...
            if (outval)
            {
                ...
                else if (door->thinker.function_idx == THINKER_IDX(T_PlatRaise))
                {
