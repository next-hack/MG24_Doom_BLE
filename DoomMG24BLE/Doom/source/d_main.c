...
static const char *timedemo = TIME_DEMO;// "demo3"; //NULL; //"demo1";
 ...
 *  short circuit operator madness begin!
 *
...
static void D_Display(void)
{
    ...
    { // Not a level
// TODO: remove this! We should not use drawstopy anymore!
...
static void D_DoomLoop(void)
{
...
#endif
#if COUNT_LUMP_BY_NAME_REQUESTS
    int oldLumpByNameRequests = 0;
#endif
#if COUNT_TRIG_OPS_PER_FRAME
    int oldTrigOpsPerFrame = 0;
#endif
#if DEBUG_SETUP && AUTOSTART_GAME
    ...
    _g->fps_show = SHOW_FPS_IN_AMMO;
    _g->gameStarted = true;
    SET_FLASH_MODE();   // will be used for rendering to read data from flash
    for (;;)
    {
      // frame synchronous IO operations
        ...
        //
        {
          ...
          __enable_irq();
             ...
             {
               //uint32_t n = I_GetTimeMicrosecs();
               TryRunTics();
               //n = I_GetTimeMicrosecs() - n;
               //printf ("Try RunTics %d us\r\n", n);
             }
#if STACK_CHANGE
             __disable_irq();
             __asm volatile
...
#endif
#if COUNT_TRIG_OPS_PER_FRAME
        if (oldTrigOpsPerFrame < trigOpsPerFrame)
        {
            oldTrigOpsPerFrame = trigOpsPerFrame;
            printf("Cosine calculations per frame increased to %d\r\n", trigOpsPerFrame);

        }
#endif
        if (_g->fps_show || SHOW_FPS_IN_HUD)
        {
            D_UpdateFPS();
        }
#if DEBUG_SETUP
...
static void D_UpdateFPS()
{
    ...
    unsigned int timenow = I_GetTimeMicrosecs(); //
...
static void D_PageDrawer(void)
{
    ...
    if (_g->pagelump)
    {
        uint32_t flags = VPT_STRETCH;
        if (_g->menuactive)
          flags |= VPT_DARK;
        V_DrawNumPatch(0, 0, 0, _g->pagelump, CR_DEFAULT, flags);
    }
...
{
    (void) iwad_len;
    extMemSetCurrentAddress((uint32_t) iwad_data);
    wadinfo_t header_v;
    extMemGetDataFromCurrentAddress(&header_v, sizeof(header_v));
    ...
    if (!strncmp(header->identification, "IWAD", 4))
    {
        ...
        extMemSetCurrentAddress((uint32_t) &iwad_data[header->infotableofs]);
        extMemGetDataFromCurrentAddress(fileinfo, length * sizeof(*fileinfo));
        while (length--)
        {
...
{
    if (doom_iwad && ((uint32_t) p_doom_iwad_len > 0))
    {
        uint32_t doom_iwad_len;
        extMemSetCurrentAddress((uint32_t) p_doom_iwad_len);
        extMemGetDataFromCurrentAddress(&doom_iwad_len, sizeof(doom_iwad_len));
        ...
        if (_g->gamemode == indetermined)
        {
            ...
            lprintf(LO_WARN, "Unknown Game Version, blocking\n");
            while(1);
        }
        D_AddFile();
    }
...
static void D_DoomMainSetup(void)
{
    ...
    _g->firstPatchLumpNum = W_GetNumForName("HELP");
    if (_g->firstPatchLumpNum == -1)
    {
        _g->firstPatchLumpNum = W_GetNumForName("HELP1");
    }
    if (_g->firstPatchLumpNum == -1)
    {
        _g->firstPatchLumpNum  = 0;
    }
    _g->lastPatchLumpNum =  W_GetNumForName("S_END");
    if (_g->lastPatchLumpNum == -1)
    {
        _g->lastPatchLumpNum = _g->numlumps;
    }
    printf("First patch %d, last patch %d\r\n", _g->firstPatchLumpNum, _g->lastPatchLumpNum);
    int numPatches = _g->lastPatchLumpNum - _g->firstPatchLumpNum;
    //
    patchsizeoffsets_t *patchSizeOffsets = Z_Malloc( sizeof(patchsizeoffsets_t) * numPatches, PU_STATIC, NULL);   
    for (int i = _g->firstPatchLumpNum; i < _g->lastPatchLumpNum; i++)
    {
       const patch_t *patch = W_CacheLumpNum(i);
       extMemSetCurrentAddress((uint32_t) patch);
       fullpatchsizeoffsets_t rpatch;
       extMemGetDataFromCurrentAddress(&rpatch, sizeof (fullpatchsizeoffsets_t));
       patchsizeoffsets_t *pp = &patchSizeOffsets[i - _g->firstPatchLumpNum];
       pp->height = rpatch.height;
       pp->leftoffset = rpatch.leftoffset;
       pp->topoffset = rpatch.topoffset;
       pp->width = rpatch.width;
    }
    printf("Saving %d bytes to flash (patchSizeOffsets)\r\n", sizeof(patchsizeoffsets_t) * numPatches);
    p_wad_immutable_flash_data->patchLumpSizeOffsets = writeBufferToFlashRegion(patchSizeOffsets, sizeof(patchsizeoffsets_t) * numPatches, FLASH_IMMUTABLE_REGION, true);
    Z_Free(patchSizeOffsets);
    //
    p_wad_immutable_flash_data = storeImmutableDataHeader(); // after this call no modification of immutable values is possible without erasing.
}
